/****** Object:  StoredProcedure [dbo].[dch_pAddProfileKey]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Prodecure adds new records to TProfileKey table returning ProfileKeyID.
--	If record with the same FIXED value already exists, then it just returns
--	it's ProfileKeyID. It also adds (or changes) respective TProfileSetting record.
CREATE PROCEDURE [dbo].[dch_pAddProfileKey](
	@ProfileKeyID		BIGINT OUT,
	@ProfileKeyGroupID	BIGINT = NULL,
	@Fixed				NVARCHAR(100),
	@ProfileKey			NVARCHAR(255) = NULL,
	@Description		NVARCHAR(MAX) = NULL,
	@DefaultValue		NVARCHAR(255) = NULL,
	@DataType			NVARCHAR(50)  = NULL,
	@SettingValue		NVARCHAR(2048) = '', 
	@IsOverridable		BIT = 1,
	@IsAdminRequired	BIT = 0
	)
AS
BEGIN
	SET	NOCOUNT		ON
	...
	DECLARE	@ROWCOUNT			BIGINT,
			...
			@OldSettingValue	NVARCHAR(2048)
	--	Check/add key
	SELECT	@ProfileKeyID	= ProfileKeyID
	  FROM	TProfileKey
	 WHERE	FIXED			= @Fixed
	...
	IF @ROWCOUNT	= 0
	BEGIN
		INSERT INTO TProfileKey( PROFILEKEYGROUPID, FIXED, PROFILEKEY, DESCRIPTION, DEFAULTVALUE, DATATYPE)
			VALUES( @ProfileKeyGroupID, @Fixed, @ProfileKey, @Description, @DefaultValue, @DataType)
		SET	@ProfileKeyID	= SCOPE_IDENTITY()
		PRINT 'ProfileKey added. FIXED = ['+@Fixed+'].'
	END
	ELSE IF @ROWCOUNT = 1
		PRINT N'*  ProfileKey having FIXED=['+@Fixed+'] already exists.'
	ELSE BEGIN
		RAISERROR('*  Few ProfileKeys having FIXED=[%s] already exist.', 16, 1, @Fixed)
		RETURN
	END
	PRINT 'ProfileKeyID = '+CAST(@ProfileKeyID AS VARCHAR)

	--	Check/add setting
	SELECT  @ProfileSettingID	= ProfileSettingID,
			@OldSettingValue	= SettingValue
	  FROM	TProfileSetting		PS
	 WHERE	PS.ProfileKeyID		= @ProfileKeyID
	...
	IF	@ROWCOUNT	= 0
	BEGIN
		INSERT INTO TPROFILESETTING( [USERGROUPID], [USERID], [PROFILEKEYID], [PROFILEBLOBID], [SETTINGVALUE], [ISOVERRIDABLE], [ISADMINREQUIRED])
			VALUES( NULL, NULL, @ProfileKeyID, NULL, @SettingValue, @IsOverridable, @IsAdminRequired)
		SET	@ProfileSettingID	= SCOPE_IDENTITY()
		PRINT 'ProfileSetting added. SettingValue = ['+ISNULL(@SettingValue, 'NULL')+'].'
	END
	ELSE IF @ROWCOUNT = 1
	BEGIN
		PRINT N'*  ProfileSetting for key having FIXED=['+@Fixed+'] already exists.'
		IF	(	@OldSettingValue IS NULL
			AND	@SettingValue IS NOT NULL)
		OR	(	@OldSettingValue IS NOT NULL
			AND	@SettingValue IS NOT NULL
			AND @SettingValue != ''
			AND	@OldSettingValue != @SettingValue)
		BEGIN
			UPDATE	TProfileSetting
			   SET	SettingValue		= @SettingValue
			 WHERE	ProfileSettingID	= @ProfileSettingID
			PRINT N'*  Value has been changed from ['+@OldSettingValue+'] to ['+@SettingValue+'].'
		END
	END
	ELSE BEGIN
		RAISERROR('*  Few ProfileSettings  for key having FIXED=[%s] already exist.', 16, 1, @Fixed)
		RETURN
	END
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pAddProfileKeyGroup]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--------------------------------------------------------------------------------
--	Contiki profile groups and keys management procedures
--------------------------------------------------------------------------------
--	Procedure adds new record to TProfileKeyGroup table returning ProfileKeyGroupID. 
--	If record with the same FIXED value already exists, then it just returns it's 
--	ProfileKeyGroupID
CREATE PROCEDURE [dbo].[dch_pAddProfileKeyGroup](
	@Fixed				NVARCHAR(255),
	@ProfileKeyGroup	NVARCHAR(255) = NULL,
	@Description		NVARCHAR(255) = NULL,
	@ProfileKeyGroupID	BIGINT OUT
	)
AS
BEGIN
	SET	NOCOUNT		ON
	...
	SELECT	@ProfileKeyGroupID	= ProfileKeyGroupID
	  FROM	TProfileKeyGroup
	 WHERE	FIXED			= @Fixed
	...
	IF @ROWCOUNT	= 0
	BEGIN
		INSERT INTO TProfileKeyGroup( FIXED, PROFILEKEYGROUP, DESCRIPTION)
			VALUES( @Fixed, @ProfileKeyGroup, @Description)

		SET	@ProfileKeyGroupID	= SCOPE_IDENTITY()
		PRINT 'ProfileKeyGroup added. FIXED = ['+@Fixed+'].'
	END
	ELSE IF @ROWCOUNT = 1
		PRINT N'*  ProfileKeyGroup having FIXED=['+@Fixed+'] already exists.'
	ELSE BEGIN
		RAISERROR('*  Few ProfileKeyGroups having FIXED=[%s] already exist.', 16, 1, @Fixed)
		RETURN
	END
	PRINT 'ProfileKeyGroupID = '+CAST(@ProfileKeyGroupID AS VARCHAR)
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pAddProfileValidValue]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[dch_pAddProfileValidValue](
	@ProfileKeyID		BIGINT,
	@Fixed				NVARCHAR(100),
	@ValidValue			NVARCHAR(255),
	@ProfileValidValue	NVARCHAR(255)
	)
AS
BEGIN
	SET	NOCOUNT		ON
	...
	DECLARE	@ROWCOUNT				BIGINT,
			@ProfileValidValueID	BIGINT

	SELECT  @ProfileValidValueID	= ProfileValidValueID
	  FROM	TProfileValidValue
	...
	IF	@ROWCOUNT	= 0
	BEGIN
		INSERT INTO TPROFILEVALIDVALUE( ProfileKeyID, Fixed, ValidValue, ProfileValidValue)
			VALUES( @ProfileKeyID, @Fixed, @ValidValue, @ProfileValidValue)
		PRINT '*  ProfileValidValue added for ProfileKeyID = '+CAST(@ProfileKeyID AS VARCHAR)+'.'
		PRINT 'Fixed = ['+ISNULL(@Fixed, 'NULL')+'], ValidValue = ['+@ValidValue+'].'
	END
	ELSE IF @ROWCOUNT = 1
	BEGIN
		PRINT N'*  ProfileValidValue having FIXED=['+@Fixed+'] already exists.'
	END
	ELSE BEGIN
		RAISERROR('*  Few ProfileValidValues  having FIXED=[%s] already exist.', 16, 1, @Fixed)
		RETURN
	END
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pAddWarningDefault]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--------------------------------------------------------------------------------
--	Contiki WARNINGDEFAULTS management procedures
--------------------------------------------------------------------------------
--	Procedure adds WarningDefaults if there is no one having the same ObjectTypeID
--	and WarningFieldName
CREATE PROCEDURE [dbo].[dch_pAddWarningDefault](
	@WARNINGDEFAULTS			NVARCHAR(255) = NULL,
	@OBJECTTYPEID				BIGINT,
	@WARNINGFIELDNAME			NVARCHAR(30),
	@WARNINGFIELDDISPLAYNAME	NVARCHAR(100) = NULL,
	@WARNINGLIMITAMOUNTID		BIGINT = NULL,
	@WARNINGLIMITOPERATOR		NVARCHAR(20) = NULL,
	@ISRECURRING				BIT = NULL,
	@WARNINGTEXT				NVARCHAR(255) = NULL,
	@PRIORWARNING				INT = NULL,
	@RECURRENCEINTERVAL			INT = NULL,
	@RECURRENCEXML				NTEXT = NULL,
	@ISLISTEDINROLES			BIT = NULL,
	@MIK_SEQUENCE				BIGINT = NULL,
...
BEGIN
	...
	DECLARE	@ROWCOUNT			BIGINT,
			@WarningDefaultsID	BIGINT

	SELECT	@WarningDefaultsID	= WarningDefaultsID
	  FROM	TWarningDefaults
	 WHERE	OBJECTTYPEID		= @OBJECTTYPEID
	   AND	WARNINGFIELDNAME	= @WARNINGFIELDNAME

	SET	@ROWCOUNT = @@ROWCOUNT

	IF @ROWCOUNT	= 0
	BEGIN
		INSERT INTO TWARNINGDEFAULTS(
			WARNINGDEFAULTS			,
			OBJECTTYPEID			,
			WARNINGFIELDNAME		,
			WARNINGFIELDDISPLAYNAME	,
			WARNINGLIMITAMOUNTID	,
			WARNINGLIMITOPERATOR	,
			ISRECURRING				,
			WARNINGTEXT				,
			PRIORWARNING			,
			RECURRENCEINTERVAL		,
		...
		SET	@WarningDefaultsID	= SCOPE_IDENTITY()
		PRINT 'WarningDefaults added. ObjectTypeID = ['+CAST(@OBJECTTYPEID AS VARCHAR)+'], WarningFieldName = ['+@WARNINGFIELDNAME+'].'
	END
	ELSE IF @ROWCOUNT = 1
		PRINT N'*  WarningDefaults having ObjectTypeID = ['+CAST(@OBJECTTYPEID AS VARCHAR)+'] and WarningFieldName = ['+@WARNINGFIELDNAME+'] already exists.'
	ELSE BEGIN
		RAISERROR('*  Few WarningDefaults having ObjectTypeID = [%d] and WarningFieldName = [%s] already exists.', 16, 1, @OBJECTTYPEID, @WARNINGFIELDNAME)
		RETURN
	END
	PRINT 'WarningDefaultsID = '+CAST(@WarningDefaultsID AS VARCHAR)
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pChangeProfileKey]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Prodecure change records in TProfileKey and TProfileSetting tables based on setting @Fixed parameter
CREATE PROCEDURE [dbo].[dch_pChangeProfileKey](
	@Fixed				NVARCHAR(100),
	@ProfileKey			NVARCHAR(255) = NULL,
	@Description		NVARCHAR(MAX) = NULL,
	@DefaultValue		NVARCHAR(255) = NULL,
...
BEGIN
	...
	BEGIN
		...
		SET PROFILEKEY = @ProfileKey,
			...
			DATATYPE = @DataType
        WHERE ProfileKeyID = @ProfileKeyID
        PRINT 'ProfileKey modified. FIXED = ['+@Fixed+'].'
	END
	ELSE IF @ROWCOUNT	= 0 
	BEGIN
		RAISERROR('*  ProfileKeys having FIXED=[%s] does not exist.', 16, 1, @Fixed)
		RETURN
	END
	ELSE BEGIN
		RAISERROR('*  Few ProfileKeys having FIXED=[%s] exist.', 16, 1, @Fixed)
		RETURN
	END
	...
	BEGIN
		INSERT INTO TPROFILESETTING( [USERGROUPID], [USERID], [PROFILEKEYID], [PROFILEBLOBID], [SETTINGVALUE], [ISOVERRIDABLE], [ISADMINREQUIRED])
			VALUES( NULL, NULL, @ProfileKeyID, NULL, @SettingValue, @IsOverridable, @IsAdminRequired)
		SET	@ProfileSettingID	= SCOPE_IDENTITY()
		PRINT 'System ProfileSetting added. SettingValue = ['+ISNULL(@SettingValue, 'NULL')+'].'
	END
	ELSE IF @ROWCOUNT = 1
	BEGIN
		IF	(	@OldSettingValue IS NULL
...
--			AND @SettingValue != ''
		...
		BEGIN
			UPDATE	TProfileSetting
			   SET	SettingValue		= @SettingValue
			 WHERE	ProfileSettingID	= @ProfileSettingID
			PRINT N'*  System setting value has been changed from ['+@OldSettingValue+'] to ['+@SettingValue+'].'
		END
	END
	ELSE BEGIN
		RAISERROR('*  Several system ProfileSettings for key having FIXED=[%s] already exist.', 16, 1, @Fixed)
		RETURN
	END
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pColumnInfo]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Procedure returns column's properties
--	it returns @OrdinalPosition = NULL if column does not exist
CREATE PROCEDURE [dbo].[dch_pColumnInfo](
		@TableName				VARCHAR(256),
		@ColumnName				VARCHAR(256),
		@OrdinalPosition		INT					OUT,
		@ColumnDefault			NVARCHAR(4000)		OUT,
		@IsNullable				VARCHAR(3)			OUT,
		@DataType				NVARCHAR(128)		OUT,
		@CharacterMaximumLength	INTEGER				OUT,
		@NumericPrecision		TINYINT				OUT,
		@NumericScale			INT					OUT,
		@DateTimePrecision		SMALLINT			OUT,
		@CollationName			NVARCHAR(128)		OUT
	)
AS
BEGIN
	IF	dbo.dch_fTableExists( @TableName)	!= 1
	BEGIN
...
GO
/****** Object:  StoredProcedure [dbo].[dch_pDeleteProfileKey]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

--	Prodecure deletes existing records from TProfileKey.
--	It also deletes respective TProfileSetting, TProfileBLOB & TProfileValidValue record.
CREATE PROCEDURE [dbo].[dch_pDeleteProfileKey](
	@ProfileKeyFixed	NVARCHAR(100)
	)
AS
BEGIN
	...
	SELECT TOP 1 @PROFILEKEYID = PROFILEKEYID FROM dbo.TPROFILEKEY WHERE FIXED=@ProfileKeyFixed

	IF @PROFILEKEYID IS NULL
	BEGIN
		PRINT N'ProfileKey for key having FIXED=' + @ProfileKeyFixed + ' doesn''t exist.' 
		RETURN
	END

	DECLARE @blobTable TABLE(BLOBID BIGINT)
	INSERT INTO @blobTable 
		SELECT PROFILEBLOBID FROM dbo.TPROFILESETTING WHERE PROFILEKEYID=@PROFILEKEYID

	delete from dbo.TPROFILESETTING where PROFILEKEYID=@PROFILEKEYID
	SET	@ROWCOUNT = @@ROWCOUNT
	PRINT CAST(@ROWCOUNT AS VARCHAR) + N' records from TPROFILESETTING were deleted'

	DELETE FROM dbo.TPROFILEBLOB WHERE PROFILEBLOBID IN (SELECT BLOBID FROM @blobTable)
	SET	@ROWCOUNT = @@ROWCOUNT
	PRINT CAST(@ROWCOUNT AS VARCHAR) + N' records from TPROFILEBLOB were deleted'

	delete from dbo.TPROFILEVALIDVALUE where PROFILEKEYID=@PROFILEKEYID
	SET	@ROWCOUNT = @@ROWCOUNT
	PRINT CAST(@ROWCOUNT AS VARCHAR) + N' records from TPROFILEVALIDVALUE were deleted'

	delete from dbo.TPROFILEKEY where PROFILEKEYID=@PROFILEKEYID
	SET	@ROWCOUNT = @@ROWCOUNT
	PRINT CAST(@ROWCOUNT AS VARCHAR) + N' record with FIXED=' + @ProfileKeyFixed + ' from TPROFILEKEY was deleted'
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pDeleteProfileKeyGroup]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

--	Prodecure deletes existing records from TProfileKeyGroup.
--	It also deletes respective TProfileKey, TProfileSetting, TProfileBLOB & TProfileValidValue records.
CREATE PROCEDURE [dbo].[dch_pDeleteProfileKeyGroup](
	@ProfileKeyGroupFixed	NVARCHAR(100)
	)
AS
BEGIN
	SET	NOCOUNT		ON
	...
	DECLARE	@ROWCOUNT	       BIGINT,
			@PROFILEKEYGROUPID BIGINT

	SELECT TOP 1 @PROFILEKEYGROUPID = PROFILEKEYGROUPID FROM dbo.TPROFILEKEYGROUP WHERE FIXED=@ProfileKeyGroupFixed

	IF @PROFILEKEYGROUPID IS NULL
	BEGIN
		PRINT N'ProfileKeyGroup for FIXED=' + @ProfileKeyGroupFixed + ' doesn''t exist.' 
		RETURN
	END

	DECLARE @keyFixed NVARCHAR(100)
	DECLARE kCursor CURSOR LOCAL STATIC FOR
		SELECT FIXED FROM dbo.TPROFILEKEY WHERE PROFILEKEYGROUPID = @PROFILEKEYGROUPID
	OPEN kCursor
	FETCH NEXT FROM kCursor INTO @keyFixed
	WHILE @@FETCH_STATUS = 0 BEGIN
		EXEC dch_pDeleteProfileKey @keyFixed
	    FETCH NEXT FROM kCursor INTO @keyFixed
	END
	CLOSE kCursor
	DEALLOCATE kCursor

	delete from dbo.TPROFILEKEYGROUP where PROFILEKEYGROUPID = @PROFILEKEYGROUPID
	SET	@ROWCOUNT = @@ROWCOUNT
	PRINT CAST(@ROWCOUNT AS VARCHAR) + N' record with FIXED=' + @ProfileKeyGroupFixed + ' from TPROFILEKEYGROUP was deleted'

END

GO
/****** Object:  StoredProcedure [dbo].[dch_pDropConstraint]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Procedure drops table constraint
CREATE	PROCEDURE	[dbo].[dch_pDropConstraint](
	@TableName		VARCHAR(256),	--	Table name
	@ConstraintName	VARCHAR(256)		--	Constraint name
	)
AS
BEGIN
	DECLARE
		@Statement	NVARCHAR(1024)
...
GO
/****** Object:  StoredProcedure [dbo].[dch_pDropFKIfExists]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Procedure checks if foreign key constraint exists and drops it if so
CREATE	PROCEDURE	[dbo].[dch_pDropFKIfExists](
...
BEGIN
	DECLARE
		@ConstraintName	VARCHAR(265)

	EXEC	@ConstraintName	= dch_fGetFKName
	...
	IF @ConstraintName	IS NOT NULL
		EXEC	dch_pDropConstraint
			@TableName		= @DetailTable,
			@ConstraintName	= @ConstraintName
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pDropIndex]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--	Procedure drops index if exists,
--	otherwise it does nothing
CREATE	PROCEDURE	[dbo].[dch_pDropIndex](
	@TableName		VARCHAR(256),
	@Column1		VARCHAR(256),
	@Column2		VARCHAR(256) = NULL,
	@Column3		VARCHAR(256) = NULL,
	@Column4		VARCHAR(256) = NULL,
...
BEGIN
	...
	BEGIN
		SET	@Statement	= 'DROP INDEX '+@TableName+'.'+@IndexName
		EXEC	SP_EXECUTESQL	@Stmt	= @Statement
	END
END

GO
/****** Object:  StoredProcedure [dbo].[dch_pGetIndexName]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--------------------------------------------------------------------------------
--	Index management procedures and functions
--------------------------------------------------------------------------------
--	Procedure returns index name if exists,
--	otherwise it returns NULL value
CREATE	PROCEDURE	[dbo].[dch_pGetIndexName](
	@TableName		VARCHAR(256),
	@Column1		VARCHAR(256),
	@Column2		VARCHAR(256) = NULL,
	@Column3		VARCHAR(256) = NULL,
...
BEGIN
	...
	INSERT	INTO	@FieldsTab	VALUES( 1, @Column1)
	IF @Column2	IS NOT NULL
	BEGIN
		INSERT	INTO	@FieldsTab	VALUES( 2, @Column2)
		IF	@Column3	IS NOT NULL
		BEGIN
			INSERT	INTO	@FieldsTab	VALUES( 3, @Column3)
			IF	@Column4	IS NOT NULL
			BEGIN
				INSERT	INTO	@FieldsTab	VALUES( 4, @Column4)
				IF	@Column5	IS NOT NULL
				BEGIN
					INSERT	INTO	@FieldsTab	VALUES( 5, @Column5)
				END
			END
		END
	END

	IF	OBJECT_ID('TEMPDB..#IndexTab', 'U') IS NULL
	BEGIN
		CREATE TABLE #IndexTab(
				INDEX_NAME			SYSNAME,
	...
	INSERT	INTO	#IndexTab
		EXEC	SP_HELPINDEX @OBJNAME = @TableName

	SELECT	@RowCount	= COUNT(*)
	  FROM	@FieldsTab

	DECLARE	CR_IND CURSOR LOCAL STATIC FOR
		SELECT	INDEX_NAME
		  FROM	#IndexTab
			...
			SELECT	COUNT(*)
			  ...
			  JOIN	(
					 SELECT	SIK.KEYNO,
					  ...
					  JOIN	SYSINDEXES						SI
						ON	SI.INDID						= SIK.INDID
					  JOIN	INFORMATION_SCHEMA.COLUMNS		C
						ON	C.ORDINAL_POSITION				= SIK.COLID
					 WHERE	SIK.ID			= OBJECT_ID(@TableName, 'U')
					   AND	SI.ID			= OBJECT_ID(@TableName, 'U')
					   AND	C.TABLE_NAME	= @TableName
					   AND	SI.NAME			= @IndName
					)				IT
				ON	IT.KEYNO		= FT.FieldOrder
			   AND	IT.COLUMN_NAME	= FT.FieldName COLLATE DATABASE_DEFAULT
			)
		BEGIN
			IF	@RowCount	= (
					 SELECT	COUNT(*)
...
GO
/****** Object:  StoredProcedure [dbo].[kasp_change_db_size]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[kasp_change_db_size]
	@DBFileName nvarchar(128) = null,
	@LogFiles bit = 0,
	@DataFiles bit = 1,
	@PercentIncrease float = 10,
	@SizeIncrease int = null,
	@UnitsIncrease nvarchar(2) = 'MB',
	@ExecuteScript bit = 0
AS
BEGIN

	DECLARE @sql nvarchar(3960)
	DECLARE @error_msg nvarchar(4000)
	DECLARE @NewSize nvarchar(40)

	IF ((ISNULL(@DBFileName, '') != '') AND (NOT EXISTS (SELECT * FROM sysfiles WHERE name = @DBFileName)))
	BEGIN

		SELECT @error_msg = 'File ''' + @DBFileName + ''' not found!'
	...
	IF (ISNULL(@DBFileName, '') != '')
	BEGIN

		IF ((ISNULL(@SizeIncrease, 0) = 0) AND (ISNULL(@PercentIncrease, 0) != 0))
		BEGIN

			SELECT 
				@NewSize = CAST(
					(CASE
						ROUND(((size * cast(8192 as bigint) / CAST((1024 * 1024) AS FLOAT)) * (1 + @PercentIncrease / 100)), 0)

					WHEN 
						ROUND((size * cast(8192 as bigint) / CAST((1024 * 1024) AS FLOAT)), 0) 
					THEN
						ROUND((size * cast(8192 as bigint) / CAST((1024 * 1024) AS FLOAT)), 0) + 1
		...
		ELSE
		BEGIN
			IF ((ISNULL(@SizeIncrease, 0) != 0) AND ((@UnitsIncrease = N'KB') OR (@UnitsIncrease = N'MB') OR (@UnitsIncrease = N'GB')))
			BEGIN
				SELECT @NewSize = 
					(CASE @UnitsIncrease
					 WHEN N'KB'
					 THEN
						CAST(CAST(ROUND(((size * cast(8192 as bigint) / CAST((1024) AS FLOAT)) + @SizeIncrease), 0) AS bigint) AS nvarchar(38)) + @UnitsIncrease
					 WHEN N'MB'
					 THEN
						CAST(CAST(ROUND(((size * cast(8192 as bigint) / CAST((1024 * 1024) AS FLOAT)) + @SizeIncrease), 0) AS bigint) AS nvarchar(38)) + @UnitsIncrease
					 WHEN N'GB'
					 THEN
						CAST(CAST(ROUND(((size * cast(8192 as bigint) / CAST((1024 * 1024 * 1024) AS FLOAT)) + @SizeIncrease), 0) AS bigint) AS nvarchar(38)) + @UnitsIncrease
					 END)
				FROM
					sysfiles
				WHERE
			...
			ELSE
			BEGIN
				RAISERROR('Size increment not specified!', 16, 1)
				RETURN(-1)
			END
		END
	...
	BEGIN
		...
		IF ((@LogFiles = 1) AND (@DataFiles = 1))
			DECLARE files_for_autogrow_cursor CURSOR FAST_FORWARD
			FOR
				SELECT
					name
				FROM
		...
		ELSE
			...
			ELSE
				...
				BEGIN
					...
					RAISERROR('No files (data or log) are selected to be resized!', 16, 1)
					RETURN(-1)

				END

		OPEN files_for_autogrow_cursor

		SELECT @sql = ''

		DECLARE @FileName nvarchar(128)

		FETCH NEXT FROM files_for_autogrow_cursor INTO @FileName
		...
		BEGIN
			...
			BEGIN
				...
				SELECT @sql = @sql +
					'ALTER DATABASE ' + DB_NAME() + ' '/*CHAR(10)*/ +
					'MODIFY FILE' + ' '/*CHAR(10)*/ +
					'	(NAME = ' + RTRIM(@FileName) + ',' + ' '/*CHAR(10)*/ +
					'	 SIZE = ' + @NewSize + ')' + ' '/*CHAR(10)*/ +
					''/*'GO'*/ + ' '/*CHAR(10)*/ + ' '/*CHAR(10)*/
		...
		CLOSE files_for_autogrow_cursor
		DEALLOCATE files_for_autogrow_cursor
...
END
GO
/****** Object:  StoredProcedure [dbo].[kasp_check_db_size]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[kasp_check_db_size]
	@CriticalFreeSpacePercent int = 10
AS
BEGIN
	...
	IF (EXISTS
		(select 
			*,
			CAST(size AS float) / CAST(FILEPROPERTY(name, 'SpaceUsed') AS float)
		from
			sysfiles
		WHERE
			groupid > 0 AND
			CAST(size AS float) / CAST(FILEPROPERTY(name, 'SpaceUsed') AS float) < (1 + CAST(@CriticalFreeSpacePercent AS float) / 100)))
	BEGIN

		RAISERROR('Some files have insufficient space.', 16, 1)
		RETURN(-1)
...
END
GO
/****** Object:  StoredProcedure [dbo].[kasp_script_table_constraints]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[kasp_script_table_constraints]
	@table_schema_input nvarchar(256) = null,
	@table_name_input nvarchar(256) = null,
	@drop_scripts bit = 0,
	@add_scripts bit = 0,
	@add_nocheck bit = 0,
	@check_existing bit = 0,
	@defaults bit = 0,
	@unique bit = 0,
	@check bit = 0,
	@foreign bit = 0,
	@foreign_parent bit = 0,
	@primary bit = 0,
	@execute_scripts bit = 0,
	@go_end_batch nvarchar(255) = 'GO'
AS
BEGIN
	...
	DECLARE @ChildForeignKey nvarchar(128)
	DECLARE @ChildColumn nvarchar(128)
	DECLARE @ParentTable nvarchar(128)
	DECLARE @ParentKey nvarchar(128)
	DECLARE @ParentColumn nvarchar(128)
	DECLARE @sql nvarchar(4000)
	DECLARE @PrimaryKeyColumn nvarchar(128)
	DECLARE @ForeignKeyColumn nvarchar(128)
	DECLARE @PrimaryKeyColumnList nvarchar(2000)
	DECLARE @ForeignKeyColumnList nvarchar(2000)
	DECLARE @IsClusteredKey int
	DECLARE @ConstraintID int
	DECLARE @UpdateRule varchar(9)
	DECLARE @DeleteRule varchar(9)
	DECLARE @go char(4)

	DECLARE @table_name sysname
	DECLARE @table_schema nvarchar(256)

	IF (@execute_scripts = 0)
		SET @go = CHAR(10) + @go_end_batch + CHAR(10)
	ELSE
		SET @go = ''

	-- Result table

	CREATE TABLE #result (row_num int identity (1, 1), row_text nvarchar(4000))

	-- Cursor on tables

	IF ((ISNULL(@table_schema_input, '') = '') AND (ISNULL(@table_name_input, '') = ''))
	...
	ELSE
		...
		FOR
			...
			WHERE
				type = N'U' AND
				USER_NAME(uid) = @table_schema_input AND
				name = @table_name_input AND
				name != 'dtproperties'
			ORDER BY
				name
	...
	BEGIN
		...
		IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--==================================================================================================')
		IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	DROP SCRIPTS')

		IF (@defaults = 1)
		BEGIN

			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	DEFAULTS')
			...
			BEGIN
				...
				SELECT
					(CASE @check_existing 
					WHEN 1 THEN 
						'IF (EXISTS (SELECT * FROM sysconstraints WHERE constid = OBJECT_ID(''' + LTRIM(RTRIM(so.name)) + ''') AND ' +
						'id = OBJECT_ID(''' + LTRIM(RTRIM(OBJECT_NAME(so.parent_obj))) + ''')))' + CHAR(10) + CHAR(9)
					ELSE
						''
					END) +
					'ALTER TABLE ' + LTRIM(RTRIM(OBJECT_NAME(so.parent_obj))) + ' DROP CONSTRAINT ' + LTRIM(RTRIM(so.name)) + @go
				FROM
					sysobjects AS so
				WHERE 
					OBJECT_NAME(so.parent_obj) = @table_name AND
		...
		BEGIN
			...
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	UNIQUE')
			...
			BEGIN
				...
				SELECT
					(CASE @check_existing 
					WHEN 1 THEN 
						'IF (EXISTS (SELECT * FROM sysconstraints WHERE constid = OBJECT_ID(''' + LTRIM(RTRIM(TC.constraint_name)) + ''') AND ' +
						'id = OBJECT_ID(''' + LTRIM(RTRIM(TC.table_name)) + ''')))' + CHAR(10) + CHAR(9)
					ELSE
						''
					END) +
					'ALTER TABLE ' + LTRIM(RTRIM(RTRIM(TC.table_schema) + '.' + TC.table_name)) + ' DROP CONSTRAINT ' + LTRIM(RTRIM(TC.constraint_name)) + @go
				FROM
		...
		BEGIN
			...
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	CHECK')
		...
		BEGIN
			...
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	FOREIGN KEYS')
				...
				IF (@foreign_parent = 0)
					...
					FROM 
						INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS TC
							...
							INNER JOIN 
							INFORMATION_SCHEMA.CONSTRAINT_TABLE_USAGE AS CTU ON (RC.unique_constraint_name = CTU.constraint_name)
					
					WHERE 
						TC.constraint_type = 'FOREIGN KEY' AND
						CTU.table_schema = @table_schema AND
		...
		BEGIN
			...
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
			IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	PRIMARY KEY')
	...
	BEGIN
		...
		IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--==================================================================================================')
		IF (@execute_scripts = 0) INSERT INTO #result(row_text) VALUES ('--	ADD SCRIPTS')
		...
		BEGIN
			...
			DECLARE pk_add_cursor CURSOR FAST_FORWARD
			FOR
				
				SELECT
					RTRIM(TC.table_schema) + '.' + TC.table_name AS ParentTable,
					TC.constraint_name AS PrimaryKeyName,
					KCU.column_name AS ColumnName,
					OBJECTPROPERTY(OBJECT_ID(TC.constraint_name), 'CnstIsClustKey') AS IsClusteredKey
				FROM
					INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS TC
						INNER JOIN 
			...
			IF (@foreign_parent = 0)
				...
				DECLARE fk_cursor CURSOR FAST_FORWARD
				FOR
			...
			DECLARE unique_add_cursor CURSOR FAST_FORWARD
			FOR
		
				SELECT
					RTRIM(TC.table_schema) + '.' + TC.table_name AS ParentTable,
					TC.constraint_name AS UniqueName,
					NULL,
				--	KCU.column_name AS ColumnName,
					OBJECTPROPERTY(OBJECT_ID(TC.constraint_name), 'CnstIsClustKey') AS IsClusteredKey
				FROM
			...
			DECLARE check_cursor CURSOR FAST_FORWARD
			FOR
			...
			DECLARE defaults_cursor CURSOR FAST_FORWARD
			FOR
		
				SELECT
					so.id AS ConstraintID,
					OBJECT_NAME(so.parent_obj) AS ParentTable,
					so.name AS ParentKey,
					(SELECT name FROM syscolumns WHERE cdefault = so.id) AS ParentColumn
				FROM
					sysobjects AS so
				WHERE 
					OBJECT_NAME(so.parent_obj) = @table_name AND
			...
			BEGIN
				...
				FETCH NEXT FROM defaults_cursor INTO @ConstraintID, @ParentTable, @ParentKey, @ParentColumn
			
				IF @@FETCH_STATUS = 0 AND (@execute_scripts = 0)
				BEGIN
		
					INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
					INSERT INTO #result(row_text) VALUES ('--	DEFAULTS')
				...
				BEGIN
					...
					IF (@check_existing = 1)
						SET @sql = 
							'IF (NOT EXISTS (SELECT * FROM sysconstraints WHERE constid = OBJECT_ID(''' + LTRIM(RTRIM(@ParentKey)) + ''') AND ' +
							'id = OBJECT_ID(''' + SUBSTRING(LTRIM(RTRIM(@ParentTable)), CHARINDEX('.', LTRIM(RTRIM(@ParentTable))) + 1, LEN(LTRIM(RTRIM(@ParentTable))) - CHARINDEX('.', LTRIM(RTRIM(@ParentTable)))) + ''')))'

					SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' ADD CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + ' DEFAULT' + CHAR(10)
					...
					SELECT @sql = text/*SUBSTRING(text, 2, LEN(text)-2)*/ FROM syscomments WHERE id = @ConstraintID

					UPDATE
						#result
					SET
						row_text = row_text + @sql + ' FOR ' + @ParentColumn + @go
					WHERE
						row_num = SCOPE_IDENTITY()
			...
			BEGIN
				...
				OPEN unique_add_cursor
				
				FETCH NEXT FROM unique_add_cursor INTO @ParentTable, @ParentKey, @ParentColumn, @IsClusteredKey
		
				IF @@FETCH_STATUS = 0 AND (@execute_scripts = 0)
				BEGIN
		
					INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
					INSERT INTO #result(row_text) VALUES ('--	UNIQUE')
				...
				BEGIN
					...
					DECLARE unique_columns_cursor CURSOR FAST_FORWARD
					FOR
						SELECT
							KCUP.COLUMN_NAME
						FROM
							INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS KCUP
						WHERE
							KCUP.constraint_name = @ParentKey
						ORDER BY
							KCUP.ORDINAL_POSITION
				
					OPEN unique_columns_cursor
				
					FETCH NEXT FROM unique_columns_cursor INTO @PrimaryKeyColumn
				
					SELECT @PrimaryKeyColumnList = ''
				
					WHILE @@FETCH_STATUS = 0
					BEGIN
				
						SET @PrimaryKeyColumnList = @PrimaryKeyColumnList + @PrimaryKeyColumn + ', '
						
						FETCH NEXT FROM unique_columns_cursor INTO @PrimaryKeyColumn
				
					END 
				
					CLOSE unique_columns_cursor
					DEALLOCATE unique_columns_cursor
				
					IF (LEN(@PrimaryKeyColumnList) > 1)
					BEGIN
				
						SET @PrimaryKeyColumnList = LEFT(@PrimaryKeyColumnList, LEN(@PrimaryKeyColumnList) - 1)
						...
						SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' ADD CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + ' UNIQUE'
		
						IF @IsClusteredKey = 1
							SET @sql = @sql + ' CLUSTERED (' + @PrimaryKeyColumnList
						ELSE
							SET @sql = @sql + ' NONCLUSTERED (' + @PrimaryKeyColumnList
			...
			BEGIN
				...
				FETCH NEXT FROM check_cursor INTO @ParentTable, @ParentKey
			
				IF @@FETCH_STATUS = 0 AND (@execute_scripts = 0)
				BEGIN
		
					INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
					INSERT INTO #result(row_text) VALUES ('--	CHECK')
				...
				BEGIN
					...
					IF (@add_nocheck = 1)
						SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' WITH NOCHECK ADD CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + ' CHECK' + CHAR(10)
					ELSE
						SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' ADD CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + ' CHECK' + CHAR(10)
					...
					SELECT @sql = check_clause FROM INFORMATION_SCHEMA.CHECK_CONSTRAINTS WHERE constraint_name = @ParentKey

					UPDATE
						#result
					SET 
						row_text = row_text + @sql + @go
					WHERE
						row_num = SCOPE_IDENTITY()

					IF (OBJECTPROPERTY(OBJECT_ID(@ParentKey), 'CnstIsNotRepl') = 1)
						INSERT
						INTO
							#result
							(
						...
						VALUES
						(
							'NOT FOR REPLICATION' + @go
						)
						
					IF (OBJECTPROPERTY(OBJECT_ID(@ParentKey), 'CnstIsDisabled') = 1)
						INSERT
						INTO
							#result
							(
						...
						VALUES
						(
							'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' NOCHECK CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + @go
						)
			...
			BEGIN
				...
				FETCH NEXT FROM pk_add_cursor INTO @ParentTable, @ParentKey, @ParentColumn, @IsClusteredKey
		
				IF @@FETCH_STATUS = 0 AND (@execute_scripts = 0)
				BEGIN
		
					INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
					INSERT INTO #result(row_text) VALUES ('--	PRIMARY KEY')
				...
				BEGIN
					...
					SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ParentTable)) + ' ADD CONSTRAINT ' + LTRIM(RTRIM(@ParentKey)) + ' PRIMARY KEY'
		
					IF @IsClusteredKey = 1
						SET @sql = @sql + ' CLUSTERED ('
					ELSE
						SET @sql = @sql + ' NONCLUSTERED ('
				END
		
				WHILE @@FETCH_STATUS = 0
				BEGIN
		
					SET @sql = @sql + LTRIM(RTRIM(@ParentColumn)) + ', '
			...
			BEGIN
				...
				FETCH NEXT FROM fk_cursor INTO @ChildTable, @ChildForeignKey, @ChildColumn, @ParentTable, @ParentKey, @ParentColumn, @UpdateRule, @DeleteRule
				
				IF @@FETCH_STATUS = 0 AND (@execute_scripts = 0)
				BEGIN
		
					INSERT INTO #result(row_text) VALUES ('----------------------------------------------------------------------------------------------------')
					INSERT INTO #result(row_text) VALUES ('--	FOREIGN KEYS')
				...
				BEGIN
					...
					DECLARE fk_columns_cursor CURSOR FAST_FORWARD
					FOR
						SELECT
							KCUP.COLUMN_NAME,
							KCUF.COLUMN_NAME/*,
						...
						FROM
							INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS KCUP
								INNER JOIN
								INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS KCUF ON (KCUP.ORDINAL_POSITION = KCUF.ORDINAL_POSITION)
						WHERE
							KCUP.constraint_name = @ParentKey AND
							KCUF.constraint_name = @ChildForeignKey
						ORDER BY
							KCUP.ORDINAL_POSITION
				
					OPEN fk_columns_cursor
				
					FETCH NEXT FROM fk_columns_cursor INTO @PrimaryKeyColumn, @ForeignKeyColumn
				
					SELECT @PrimaryKeyColumnList = '', @ForeignKeyColumnList = ''
				
					WHILE @@FETCH_STATUS = 0
					BEGIN
				
						SET @PrimaryKeyColumnList = @PrimaryKeyColumnList + @PrimaryKeyColumn + ', '
						SET @ForeignKeyColumnList = @ForeignKeyColumnList + @ForeignKeyColumn + ', '
					...
					IF ((LEN(@PrimaryKeyColumnList) > 1) AND (LEN(@ForeignKeyColumnList) > 1))
					BEGIN
				
						SET @PrimaryKeyColumnList = LEFT(@PrimaryKeyColumnList, LEN(@PrimaryKeyColumnList) - 1)
						SET @ForeignKeyColumnList = LEFT(@ForeignKeyColumnList, LEN(@ForeignKeyColumnList) - 1)
				
						SET @sql = ''

						IF (@check_existing = 1)
							SET @sql = 
								'IF (NOT EXISTS (SELECT * FROM sysconstraints WHERE constid = OBJECT_ID(''' + LTRIM(RTRIM(@ChildForeignKey)) + ''') AND ' +
								'id = OBJECT_ID(''' + SUBSTRING(LTRIM(RTRIM(@ChildTable)), CHARINDEX('.', LTRIM(RTRIM(@ChildTable))) + 1, LEN(LTRIM(RTRIM(@ChildTable))) - CHARINDEX('.', LTRIM(RTRIM(@ChildTable)))) + ''')))'

						IF (@add_nocheck = 1)
							SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ChildTable)) + ' WITH NOCHECK ADD CONSTRAINT ' + LTRIM(RTRIM(@ChildForeignKey)) + 
								' FOREIGN KEY (' + @ForeignKeyColumnList + ') REFERENCES ' + @ParentTable + ' (' + @PrimaryKeyColumnList + ')'
						ELSE
							SET @sql = @sql + CHAR(10) + 'ALTER TABLE ' + LTRIM(RTRIM(@ChildTable)) + ' ADD CONSTRAINT ' + LTRIM(RTRIM(@ChildForeignKey)) + 
								' FOREIGN KEY (' + @ForeignKeyColumnList + ') REFERENCES ' + @ParentTable + ' (' + @PrimaryKeyColumnList + ')'
						...
						IF (@UpdateRule = 'CASCADE')
							UPDATE
								#result
							SET
								row_text = row_text + CHAR(10) + 'ON UPDATE CASCADE'
							WHERE
								row_num = SCOPE_IDENTITY()
						...
						IF (@DeleteRule = 'CASCADE')
							UPDATE
								#result
							SET
								row_text = row_text + CHAR(10) + 'ON DELETE CASCADE'
							WHERE
								row_num = SCOPE_IDENTITY()
						...
						UPDATE
							#result
						SET
							row_text = row_text + @go
						WHERE
							row_num = SCOPE_IDENTITY()


						IF (OBJECTPROPERTY(OBJECT_ID(@ChildForeignKey), 'CnstIsNotRepl') = 1)
							INSERT
							INTO
								#result
								(
						...
						IF (OBJECTPROPERTY(OBJECT_ID(@ChildForeignKey), 'CnstIsDisabled') = 1)
							INSERT
							INTO
								#result
								(
							...
							VALUES
							(
								'ALTER TABLE ' + LTRIM(RTRIM(@ChildTable)) + ' NOCHECK CONSTRAINT ' + LTRIM(RTRIM(@ChildForeignKey)) + @go
							)
			...
			DEALLOCATE pk_add_cursor
			DEALLOCATE fk_cursor
			DEALLOCATE unique_add_cursor
	...
	DECLARE result_cursor CURSOR FAST_FORWARD
	FOR
		SELECT
			row_text
		FROM 
	...
	OPEN result_cursor
	FETCH NEXT FROM result_cursor INTO @sql
...
END
GO
/****** Object:  StoredProcedure [dbo].[kasp_update_change_db_size_jobstep]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[kasp_update_change_db_size_jobstep]
AS
BEGIN
	...
	INTO
		#jobstep
	EXEC msdb..sp_help_jobstep @job_name = 'Increase DB size', @step_name = 'Increase DB size'
	
	DECLARE @command nvarchar(3200), @percent nvarchar(100)
	DECLARE @i int, @step_id int

	SELECT TOP 1 @command = command FROM #jobstep

	SELECT @i = 1
	SELECT @percent = ''
	SELECT TOP 1 @step_id = step_id FROM #jobstep

	WHILE @i < (LEN(@command) - LEN('@PercentIncrease'))
	BEGIN

		IF (SUBSTRING(@command, @i, LEN('@PercentIncrease')) = '@PercentIncrease')
		BEGIN
			SELECT @i = @i + LEN('@PercentIncrease')
			BREAK
		END
	...
	WHILE (SUBSTRING(@command, @i, 1) != ',')
	BEGIN

		IF (ISNUMERIC(SUBSTRING(@command, @i, 1)) = 1)
			SELECT @percent = @percent + SUBSTRING(@command, @i, 1)

		SELECT @i = @i + 1

	END

	IF (CAST(@percent AS int) = 1)
	BEGIN

		EXEC msdb..sp_update_alert 'Autogrow timed out', @enabled = 0
		RAISERROR('Autogrow timeouts exceeded! Please, check your hardware!', 17, 1)
		RETURN(-1)
	...
	BEGIN
		...
		SELECT @percent = @percent - 1

		SELECT @command = 'exec kasp_change_db_size
					@PercentIncrease = ' + @percent + ',
					@ExecuteScript = 1'

		exec msdb..sp_update_jobstep
			@job_name = 'Increase DB size',
			@step_id = @step_id,
			@command = @command
...
END
GO
/****** Object:  StoredProcedure [dbo].[MaintainSplitAuditTrail]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[MaintainSplitAuditTrail]
AS
BEGIN
	/* ------------------------------------------------------------------------------
	This procedure maintains the split of Contiki's Audit Trail table TAUDITTRAIL 
	into two tables:
	  1. TAUDITTRAIL_HISTORY: All audit trail entries older than 3 months
	  2. TAUDITTRAIL: All audit trail entries from 3 months ago and newer
	---
	  This procedure is to be executed once per day (at night) to maintain the split.
	  This procedure should be run outside office hours, as it can block Contiki users
	  for the entire time the script operates.
	  It is recommended that a SQL Server JOB is created to run this procedure on a
	  schedule, e.g. every night at 02:00.
	------------------------------------------------------------------------------ */

	-- FAIL if TAUDITTRAIL_HISTORY does not exist already: Contiki uses a single audit trail table (no split)
	IF	OBJECT_ID('dbo.TAUDITTRAIL_HISTORY', 'U') IS NULL
	BEGIN
		PRINT '***ERROR*** Table TAUDITTRAIL_HISTORY does not exist! Running this procedure is not appropriate.'
		RETURN
	END
	
	-- Sanity Check:  There must not be any newer records in TAUDITTRAIL_HISTORY already
	IF (SELECT COUNT(*) FROM TAUDITTRAIL_HISTORY H WHERE H.AUDITTRAILID > (SELECT MIN(AUDITTRAILID) FROM TAUDITTRAIL)) > 0
	BEGIN
		PRINT '***ERROR*** Table TAUDITTRAIL_HISTORY contains events newer than the oldest event in TAUDITTRAIL. The situation must be resolved before this procedure can be used safely.'
		RETURN
	END
	...
	-- Set cutoff date to  2 MONTHS  of history in TAUDITTRAIL, moving the older history to TAUDITTRAIL_HISTORY
	-- In the final version, we want to read the # of months from a Contiki system profile setting.
	declare @MonthsToKeep bigint
	set @MonthsToKeep = 1 /* Takeda setting, standard is 3 months */
	DECLARE @CUTOFF_DATE DATETIME
	SET @CUTOFF_DATE = DATEADD(MONTH, -@MonthsToKeep, CONVERT(DATE, GETDATE()))

	PRINT 'Moving all rows older than ' + CAST(@CUTOFF_DATE AS VARCHAR)  + ' from TAUDITTRAIL to TAUDITTRAIL_HISTORY.'
	PRINT ' '
	PRINT '  Copying from TAUDITTRAIL ...'
	-- Add to TAUDITTRAIL_HISTORY from TAUDITTRAIL
	set identity_insert TAUDITTRAIL_HISTORY on
	INSERT INTO TAUDITTRAIL_HISTORY
		  ([AUDITTRAILID],[OBJECTTYPEID],[OBJECTID],[EVENTID],[USERID],[TIME],[COMMENT],[XML],[PARENTOBJECTTYPEID],[PARENTOBJECTID],[OBJECTDESCRIPTION])
	SELECT [AUDITTRAILID],[OBJECTTYPEID],[OBJECTID],[EVENTID],[USERID],[TIME],[COMMENT],[XML],[PARENTOBJECTTYPEID],[PARENTOBJECTID],[OBJECTDESCRIPTION]
	FROM TAUDITTRAIL (NOLOCK)
	WHERE TIME < @CUTOFF_DATE

	PRINT ' '
	PRINT '  Deleting from TAUDITTRAIL ...'
	-- Remove the archived records from TAUDITTRAIL
	DELETE TAUDITTRAIL
	WHERE TIME < @CUTOFF_DATE

	PRINT ' '
	PRINT 'Rebuilding indexes on TAUDITTRAIL ...'
	ALTER INDEX ALL ON TAUDITTRAIL REBUILD
	PRINT 'Done.'
	
	Update statistics TAUDITTRAIL /* Takeda addition */
	Update statistics TAUDITTRAIL_HISTORY /* Takeda addition */
	
	COMMIT
END
GO
/****** Object:  StoredProcedure [dbo].[SP_COPYFILE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_COPYFILE] 
	@FILEID bigint,
...
--	SET XACT_ABORT ON
	...
	-- 	Here we check if the file to be copied exists
	IF (NOT EXISTS (SELECT * FROM TFILE WHERE FILEID = @fileid))
		RETURN -2

	-- 	Here we check if the file to copy to exists
	IF ((NOT EXISTS (SELECT * FROM TFILE WHERE FILEID = @DESTFILEID)) AND (@DESTFILEID IS NOT NULL))
		RETURN -3

	BEGIN TRAN
	
		DECLARE @destination_val binary(16)
		DECLARE @source_val binary(16)
		DECLARE @insertedfileId bigint

		IF (@DESTFILEID IS NULL)
		BEGIN
			INSERT INTO tfile ([file], [filetype]) SELECT '0', [filetype] FROM TFILE WHERE FILEID = @FILEID
			IF (@@ERROR != 0) GOTO Error
		
			SELECT @insertedfileId = scope_identity() -- as [scope_identity]
		END
		ELSE
		BEGIN
			UPDATE tfile 
			SET 
				[file] = '0', 
				[filetype] = (SELECT f.[filetype] FROM TFILE AS f WHERE FILEID = @FILEID)
			WHERE
				FileID = @DESTFILEID
			SELECT @insertedfileId = @DESTFILEID
		END
		
		SELECT @destination_val = TEXTPTR([FILE]) from TFILE where fileid =  @insertedfileId
		IF (@@ERROR != 0) GOTO Error

		SELECT @source_val = TEXTPTR([file]) FROM tfile where fileid = @fileid
		IF (@@ERROR != 0) GOTO Error

		UPDATETEXT TFILE.[FILE] @destination_val 0 NULL TFILE.[FILE] @source_val
		IF (@@ERROR != 0) GOTO Error
...
END
GO
/****** Object:  StoredProcedure [dbo].[SP_DELETEWORKFLOWDEFINITION]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[SP_DELETEWORKFLOWDEFINITION] @WFID BIGINT
	AS
	/*	This procedure can be called if you whish to delete a workflow definition and all related data.
			Note that all process information etc will also be deleted.
		
			Ex. on use:
			EXECUTE SP_DELETEWORKFLOWDEFINITION 62
	*/
	BEGIN
		SET XACT_ABORT ON /*Rolls back all if error. */
	
		BEGIN TRAN
		--  Delete processes
		UPDATE TWORKFLOWPROCESS SET INSTANTIATINGACTIVITYLINEID=NULL WHERE WORKFLOWDEFINITIONID=@WFID
	
		DELETE FROM DBO.TWORKFLOWSTATEVARIABLE WHERE WORKFLOWPROCESSID IN 
				(SELECT WORKFLOWPROCESSID FROM TWORKFLOWPROCESS WHERE WORKFLOWDEFINITIONID=@WFID)
	
		DELETE FROM TWORKFLOWPROCESSACTIVITYLINE WHERE WORKFLOWPROCESSACTIVITYID IN
			(SELECT WORKFLOWPROCESSACTIVITY FROM TWORKFLOWPROCESSACTIVITY WHERE WORKFLOWPROCESSID IN 
				(SELECT WORKFLOWPROCESSID FROM TWORKFLOWPROCESS WHERE WORKFLOWDEFINITIONID=@WFID)
			)
	
		DELETE FROM TWORKFLOWPROCESSACTIVITY WHERE WORKFLOWPROCESSID IN 
				(SELECT WORKFLOWPROCESSID FROM TWORKFLOWPROCESS WHERE WORKFLOWDEFINITIONID=@WFID)
	
		DELETE FROM TWORKFLOWPROCESS WHERE WORKFLOWDEFINITIONID=@WFID
	
		print 'All process information related to this workflowdefinition is deleted.'

		-- Delete process plans
		DELETE FROM TWORKFLOWPROCESSPLANACTIVITYLINE WHERE WORKFLOWPROCESSPLANACTIVITYID IN 
			(SELECT WORKFLOWPROCESSPLANACTIVITYID FROM TWORKFLOWPROCESSPLANACTIVITY WHERE WORKFLOWPROCESSPLANID IN
				(SELECT WORKFLOWPROCESSPLANID FROM TWORKFLOWPROCESSPLAN WHERE WORKFLOWDEFINITIONID=@WFID)
			)
	
		DELETE FROM TWORKFLOWPROCESSPLANACTIVITY WHERE WORKFLOWPROCESSPLANID IN
				(SELECT WORKFLOWPROCESSPLANID FROM TWORKFLOWPROCESSPLAN WHERE WORKFLOWDEFINITIONID=@WFID)

		DELETE FROM TWORKFLOWPROCESSPLAN WHERE WORKFLOWDEFINITIONID=@WFID
	
		print 'All process plan information related to this workflowdefinition is deleted.'
	
		-- Delete predefined plans
		DELETE FROM TWorkflowDefinitionPredefinedPlanActivity WHERE WorkflowDefinitionPredefinedPlanId in
			(SELECT WorkflowDefinitionPredefinedPlanId FROM TWorkflowDefinitionPredefinedPlan WHERE WORKFLOWDEFINITIONID=@WFID)
		
		DELETE FROM TWorkflowDefinitionPredefinedPlan WHERE WORKFLOWDEFINITIONID=@WFID
	
		print 'All predefined plan information related to this workflowdefinition is deleted.'
	
		-- Delete definitions
		DELETE FROM TWorkflowDefinition_in_Group WHERE WORKFLOWDEFINITIONID=@WFID
	
		DELETE FROM TWORKFLOWDEFINITIONSOURCE WHERE WORKFLOWDEFINITIONID=@WFID
	
		DELETE FROM TWORKFLOWSERVICEPARAMETER WHERE WORKFLOWSERVICEOPERATIONID IN
			(SELECT WORKFLOWSERVICEOPERATIONID FROM TWORKFLOWSERVICEOPERATION WHERE WORKFLOWSERVICEID IN
				(SELECT WORKFLOWSERVICEID FROM TWORKFLOWSERVICE WHERE WORKFLOWDEFINITIONID=@WFID)
			)
	
		DELETE FROM TWORKFLOWSERVICEOPERATION_IN_GROUP WHERE WORKFLOWSERVICEOPERATIONID IN
			(SELECT WORKFLOWSERVICEOPERATIONID FROM TWORKFLOWSERVICEOPERATION WHERE WORKFLOWSERVICEID IN
				(SELECT WORKFLOWSERVICEID FROM TWORKFLOWSERVICE WHERE WORKFLOWDEFINITIONID=@WFID)
			)
	
		DELETE FROM TWORKFLOWSERVICEOPERATION WHERE WORKFLOWSERVICEID IN
			(SELECT WORKFLOWSERVICEID FROM TWORKFLOWSERVICE WHERE WORKFLOWDEFINITIONID=@WFID)
	
		DELETE FROM TWORKFLOWSERVICE WHERE WORKFLOWDEFINITIONID=@WFID
	
		DELETE FROM TWORKFLOWDEFINITION WHERE WORKFLOWDEFINITIONID=@WFID
	
		COMMIT TRAN
		print 'This workflowdefinition is successfully deleted.'
	END
GO
...
AS
BEGIN
/* DAILY SCRIPT Contiki Support Team */
...
/*7b Remove all user Permissions for DELETE Records except Administrator user */
	...
	WHERE OBJECTID IN
	(SELECT CONTRACTID FROM TCONTRACT WHERE CONTRACTTYPEID ='13' /* DELETE */ )
	AND  USERID <> 20134 /* ADMINISTRATOR, CONTIKI */
...
END
GO
/****** Object:  StoredProcedure [dbo].[SP_JOINFILEPARTS]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[SP_JOINFILEPARTS] @FILEID bigint AS
/*	
	This stored procedure will do the following
	1.) Append fake content to TFILE.[FILE] with the corresponding @FILEID
	2.) Replace the binary data of TFILE.[FILE] with the last offset of TFILEPART.FILEPART with the corresponding @FILEID
	3.) Append the binary data of TFILE.[FILE] with binary data from the TFILEPART.FILEPART sorting DESC on OFFSET
	4.) Delete the rows in TFILEPART with the corresponding @FILEID
*/
BEGIN
SET NOCOUNT ON
SET XACT_ABORT ON
	...
	DECLARE @destination_val binary(16)
	DECLARE @source_val binary(16)
	DECLARE @filepartid bigint
	DECLARE @count int
	SELECT @count = 0

	update tfile set [file] = '0' WHERE fileid = @FILEID
	SELECT @destination_val = TEXTPTR([FILE]) from TFILE where fileid = @FILEID
--	PRINT 'step 1 - ok'
--	SELECT [filepartid], [offset] FROM tfilepart WHERE fileid = @FILEID ORDER BY [OFFSET] ASC
	DECLARE curs CURSOR FAST_FORWARD FOR SELECT [filepartid] FROM tfilepart WHERE fileid = @FILEID ORDER BY [OFFSET] DESC
--	PRINT 'step 2 - ok'
	OPEN curs
--	PRINT 'step 3 - ok'
	FETCH NEXT FROM curs INTO @filepartid
	WHILE @@FETCH_STATUS = 0
		begin
--			PRINT 'FETCH STATUS = ' + cast(@@FETCH_STATUS as varchar(15))
--			PRINT 'Appending - startoffset = ' + cast(@realoffset as varchar(15)) + ', endoffset = ' + cast(@offset as varchar(15))
--			PRINT @source_val
--			PRINT @destination_val
			SELECT @source_val = TEXTPTR(filepart) FROM tfilepart where filepartid = @filepartid
			IF @count = 0
			BEGIN
				updatetext TFILE.[FILE] @destination_val 0 1 TFILEPART.FILEPART @source_val
			END
			ELSE
			BEGIN
				updatetext TFILE.[FILE] @destination_val 0 0 TFILEPART.FILEPART @source_val
			END
	...
	DEALLOCATE curs
--	PRINT 'COMPLETE'
	DELETE FROM TFILEPART WHERE FILEID = @FILEID
...
GO
/****** Object:  StoredProcedure [dbo].[SP_SPLITINTOFILEPARTS]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*\
    End of script from "20101006_1035_alterTFILEINFO_CHANGECOMMENT.sql" file.                                                           
\*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*\
    Start of script from "20101018_1339_alterSP_SPLITINTOFILEPARTS.sql" file.                                                         
\*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*
	Added parameter @StartOffset.
	If it is not null procedure will return one filepart starting from this position
	If it is null procedure will work as usually
*/
CREATE PROCEDURE [dbo].[SP_SPLITINTOFILEPARTS]
	@FileID		BIGINT,
	@PartSize	INT	= 2097152,
...
BEGIN
	...
	DECLARE		@DestinationVal			VARBINARY(16),
			@SourceVal			VARBINARY(16),
			@FileSize			BIGINT,
			@Offset				INT,
			@LocalOffset			INT,
			@FilePartID			BIGINT,
			@LocalPart			VARBINARY(8000),
			@TempCount			BIGINT
	...
	SELECT	@SourceVal	= TEXTPTR([FILE])
	  FROM	TFILE
	 WHERE	FileID		= @FileID
	
	IF (OBJECT_ID('TEMPDB..#TEMPFILEPART', 'U')) IS NULL
		CREATE TABLE #TEMPFILEPART(FILEPART	IMAGE NULL)

	SELECT	@TempCount	= COUNT(*)
	  FROM	#TEMPFILEPART

	IF @TempCount > 1
		DELETE FROM #TEMPFILEPART

	IF @TempCount	!= 1
		INSERT INTO #TEMPFILEPART (FILEPART) VALUES(NULL)

	IF	@StartOffset IS NOT NULL
		SELECT	@OFFSET	= @StartOffset

	WHILE	@OFFSET	< @FileSize
	BEGIN
		SELECT	@LocalPart	= SUBSTRING( [FILE], @Offset+1, CASE WHEN 8000<@PartSize THEN 8000 ELSE @PartSize END)
		  FROM	TFILE
		 WHERE	FILEID		= @FILEID

		UPDATE	#TEMPFILEPART
		   SET	FILEPART	= @LocalPart

		SELECT	@DestinationVal	= TEXTPTR(FILEPART)
		  FROM  #TEMPFILEPART

		IF	@PartSize	> 8000
		BEGIN		
			SET @LocalOffset = 8000

			WHILE @LocalOffset < @PartSize
			BEGIN
				SELECT	@LocalPart	= SUBSTRING( [FILE], @Offset+@LocalOffset+1, CASE WHEN 8000<@PartSize-@LocalOffset THEN 8000 ELSE @PartSize-@LocalOffset END)
				  FROM	TFILE
				 WHERE	FILEID	= @FILEID

				UPDATETEXT #TEMPFILEPART.FILEPART @DestinationVal NULL 0 @LocalPart

				SET	@LocalOffset = @LocalOffset + 8000
			END
		END
		...
		BEGIN
			SELECT	@FILEID			AS FILEID,
				...
				DATALENGTH(FILEPART)	AS DATALENGTH
			FROM	#TEMPFILEPART
			RETURN 
		END

		INSERT INTO TFILEPART( FILEID, OFFSET, FILEPART)
			SELECT	@FILEID,
					@OFFSET,
					FILEPART
			  FROM	#TEMPFILEPART

		SET	@Offset	= @Offset	+ @PartSize
	END
END


GO
/****** Object:  StoredProcedure [dbo].[spi_ImportFileImage]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

create procedure [dbo].[spi_ImportFileImage]
(
    @ImporFile varchar(257),
    @workPATH varchar(249)='C:\TEMP'           
)
as
BEGIN

	set nocount on
	declare @ErrorCode int, @Size int, @cmd nvarchar(4000), @id int
	set @ErrorCode = 0
	exec sps_GetFileSize @ImporFile, @Size out
	
	if @Size = -1
	    return -1
	
	set @cmd = 'cd /D '+@workPATH+' & @echo 8.0 > bcp.fmt & @echo 1 >> bcp.fmt & @echo 1 SQLIMAGE 0 '+cast(@Size as varchar(10))+' "" 3 FileImage "">> bcp.fmt'
	
	exec @ErrorCode = master..xp_cmdshell @cmd, no_output
	
	if @ErrorCode <> 0
	    return -2
	
	set @cmd = 'BULK INSERT tFileImage FROM ''' + @ImporFile + ''' WITH ( formatfile = ''' + @workPATH + '\bcp.fmt'') set @id = SCOPE_IDENTITY()' 
	
	delete tFileImage where FileName = @ImporFile
	set @ErrorCode = @@error
	
	if @ErrorCode <> 0
	    return -3
	
	exec @ErrorCode = sp_executesql @cmd, N'@id int output', @id output
	
	if @ErrorCode <> 0
	    return -4
	
	update tFileImage set FileName = @ImporFile where id = @id

END
GO
/****** Object:  StoredProcedure [dbo].[sps_GetFileSize]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [dbo].[sps_GetFileSize] @File varchar(257), @Size int = null output 
as
BEGIN
	...
	insert into #getfiledetails exec master..xp_getfiledetails @File
	select @Size = Size from #getfiledetails
	set nocount off

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_0_ARIBADataLoad]

as

	begin
	/* make sure to back up old Ariba raw table and rename new table prior to running */
	/* select COUNT(*) from [dbo].[T_Takeda_Ariba_Dump_Raw] */
	/* select COUNT(*) from [dbo].[T_Takeda_Ariba_Dump_Raw_2020_07] 104000 records*/

	
	/* EXEC [dbo].[Takeda_0_ARIBADataLoad_00_SupplierData] */
	EXEC [dbo].[Takeda_0_ARIBADataLoad_01]
	EXEC [dbo].[Takeda_0_ARIBADataLoad_02]
	/* EXEC [dbo].[Takeda_0_ARIBADataLoad_03_SupplierData] - run first since used in 01?*/


	/* Optional: exec Takeda_3SATNIGHT_ProductGroupUpload_ARIBA_Description again (is executed every Saturday)
		/* runs on T_Takeda_AribaDump, populate product ids etc. that are then concatenated */ 
		*/

	end
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad_00_SupplierData]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_0_ARIBADataLoad_00_SupplierData]
...
/*
	insert into [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields] 
	(
	 [SupID_SAP]
      ,[SupName_SAP]
      ,[SupRegStatus]
	  ...
	  from T_Takeda_Ariba_SAPID_LieferantendatenChristophMatt_2021_02_22 l 
		left join [dbo].[T_Takeda_TCountries] c on l.[Country2LetterCode] = c.[CtyCode2Letter]
		WHERE vendorsap_id is not null and Name1 is not null
*/

	alter table [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country]
	alter column [SupName_SAP] varchar(150)

	update [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields]
	set [Sup_LettersNumbersOnly_UPPER] = dbo.Takeda_RemoveNonAlphaNonNumericCharacters([SupName_SAP]) 
...
/* VALID ENTRY OR NOT */
	...
	/* WHERE 
		[dbo].[Takeda_CountQuestionMarkCharChineseEtc_Varchar255] ([SupName_SAP])>1 */
	/* select * from 
	[dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields] where [Sup_Name_ValidString_FLAG] is null */

	update [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields]
	set [Sup_Name_ValidString_FLAG] = 1
	WHERE 
		[dbo].[Takeda_CountQuestionMarkCharChineseEtc_Varchar255] ([SupName_SAP]) = 0 /* no special char found */
...
/*
select 
[dbo].[Takeda_CountQuestionMarkCharChineseEtc_Varchar255] ([SupName_SAP]) as tst
, *
from [T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields]
where supid_sap = 'ACM_92517841'*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad_01]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_0_ARIBADataLoad_01]

as
/* in Ariba, run Takeda Contract Report 1960-2015 All Fields */
/* import T_Takeda_AribaDump_Raw from .csv as text file through access upload into sql server, with formatted fields RAW_FORMATTED */
	...
	/* back up old tbl */
	/*
	select * into [dbo].[T_Takeda_AribaDump_BAK]
	from T_Takeda_AribaDump /* [dbo].[T_Takeda_Ariba_Dump_Raw] */

	rename old raw dump
	/* rename T_Takeda_Ariba_Dump_Raw_FormattedFields to T_Takeda_Ariba_Dump_Raw */
	*/
		...
		alter table [dbo].[T_Takeda_Ariba_Dump_Raw]
		add [AllSupplier] varchar(255) /* for Contiki legacy contracts , since those are all 'legacy suppliers' */
		...
		/* alter table T_Takeda_Ariba_Dump_Raw
...
/*		CREATE UNIQUE CLUSTERED INDEX T_Takeda_Ariba_Dump_Raw_ProjectIDContractInternalID /* added unique oct-20 */
		ON T_Takeda_Ariba_Dump_Raw ([Project - Project Id])
		FAILS because raw table is not yet flattened!
	select [Project - Project Id] , COUNT(*) from T_Takeda_Ariba_Dump_Raw
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad_02]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[Takeda_0_ARIBADataLoad_02]
...
*/
	...
	update [dbo].[T_Takeda_Ariba_Dump_Raw]
	set [AffectedParties_LETTERSNUMBERSONLY] 
		= dbo.Takeda_RemoveNonAlphaNonNumericCharacters ([Affected Parties - Common Supplier])

	/* moved	update d
		set d.[CompanyCountry] = s.[SupCountry]
		/*,  d.[CompanyCountryID] */
		from [dbo].[T_Takeda_Ariba_Dump_Raw] d 
			inner join [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields] s 
...
/* RAW FLAT */
	...
	if OBJECT_ID('T_Takeda_Ariba_Dump_Raw_FLAT') is not null 
	drop table [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT]

	select * into [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT]
	from [dbo].[V_Takeda_Ariba_Dump_Raw_FLAT] /* [dbo].[T_Takeda_Ariba_Dump_Raw] */
	order by ContractInternalID asc

	CREATE UNIQUE CLUSTERED INDEX T_Takeda_Ariba_Dump_Raw_Flat_ContractInternalID
	ON T_Takeda_Ariba_Dump_Raw_FLAT (CONTRACTINTERNALID)

	/* make sure that nvarchar(max) is converted to 255 max len */
		alter table T_Takeda_Ariba_Dump_Raw_FLAT
		alter column [Effective Date - Date] date

		alter table T_Takeda_Ariba_Dump_Raw_FLAT
		alter column [Begin Date] date

		alter table T_Takeda_Ariba_Dump_Raw_FLAT /* fails if 'unclassified etc.' left in */
	...
	update T_Takeda_Ariba_Dump_Raw_FLAT
	...
	WHERE 
		...
		/* select max(len([Business Owner - User])) from T_Takeda_Ariba_Dump_Raw_FLAT */
			alter table T_Takeda_Ariba_Dump_Raw_FLAT
			add [All Products] nvarchar(1000)	/* no permission ? */

			drop table [dbo].[T_Takeda_AribaDump]

	select * into [dbo].[T_Takeda_AribaDump]
	from [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT] /* [dbo].[T_Takeda_Ariba_Dump_Raw] */
...
/* check match level, if to run quickly choose >5 char product name length, or 2 if more time */
	if OBJECT_ID('T_Takeda_Ariba_Dump_Raw_FLAT_AllProducts') is not null 
	drop table [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT_AllProducts]

	select * into [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT_AllProducts]
	from [dbo].[V_Takeda_Ariba_Dump_Raw_FLAT_AllProducts] /* [dbo].[T_Takeda_Ariba_Dump_Raw] */

	update [dbo].[T_Takeda_Ariba_Dump_Raw_FLAT_AllProducts] 
		set [All Products] = LTRIM(RTRIM([All Products])) /* strip off spaces */

	update f
	set f.[All Products] = p.[All Products] 
	from [T_Takeda_AribaDump] f 
		inner join T_Takeda_Ariba_Dump_Raw_FLAT_AllProducts p 
		on f.contractinternalid = p.contractinternalid

		/* tag table contract ids */
		update t
			set t.contractinternalid = a.contractinternalid
			from  [dbo].[T_Takeda_Ariba_TTAG_IN_ContractInternalID] t
			inner join T_Takeda_AribaDump a on t.contractnumber = a.contractnumber



	/* delete staging tables , Oct-2020 */

		if OBJECT_ID('T_Takeda_Ariba_Dump_Raw_FormattedFields') is not null 
			drop table T_Takeda_Ariba_Dump_Raw_FormattedFields /* and JPS */
	...
	/* Vcompany */
	if OBJECT_ID('T_Takeda_ContractData_ARB_1VCOMPANY') is not null 
		drop table T_Takeda_ContractData_ARB_1VCOMPANY

		select * into T_Takeda_ContractData_ARB_1VCOMPANY
		FROM V_Takeda_ContractData_ARB_1VCOMPANY

END

GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad_03_SupplierData]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_0_ARIBADataLoad_03_SupplierData]
...
/* daily proc 
			...
			/*,  d.[CompanyCountryID] */
			from tcompany c
				inner join [dbo].[t_takeda_vcompany] c2
...
/* SAP Country */
		...
		from [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields] s1 
				inner join /*[dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country]*/ T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields s2
				on s1.SupID_SAP = s2.SupID_SAP
		WHERE 
			s2.SupCountry is null
			and s1.SupCountry is not null

		if OBJECT_ID('T_Takeda_Ariba_Suppliers_SAPID_ValidMatchedCompanies') is not null 
		drop table T_Takeda_Ariba_Suppliers_SAPID_ValidMatchedCompanies

			select *
			into T_Takeda_Ariba_Suppliers_SAPID_ValidMatchedCompanies
			from [dbo].[T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields]
			where [Sup_Name_ValidString_FLAG] = 1
			and Sup_COMPANYID is not null /* would create dupes */

		CREATE UNIQUE CLUSTERED INDEX T_Takeda_Ariba_Suppliers_SAPID_ValidMatchedCompanies_COMPANYID
		ON T_Takeda_Ariba_Suppliers_SAPID_ValidMatchedCompanies (Sup_COMPANYID)
...
*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_ARIBADataLoad_Delta]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_0_ARIBADataLoad_Delta]
...
/* import T_Takeda_AribaDump_Raw from .csv as text file through access upload into sql server, with formatted fields RAW_FORMATTED */
	...
	/* Products: runs every Saturday, and run after import, takes 2 hrs */
	/*
	alter table [dbo].[T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta]
	alter column [Description] nvarchar(1000) /* ntext does not work in view */

		alter table T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta
	add [AllSupplier] varchar(255) /* for Contiki legacy contracts , since those are all 'legacy suppliers' */
	...
	*/
	update T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta
	...
	/* alter table T_Takeda_Ariba_Dump_Raw
	add [Affected Parties - Common Supplier LETTERS ONLY] varchar(255) /* for Contiki legacy contracts , since those are all 'legacy suppliers' */
	*/
	...
	AND */ 
		...
		/* select * from  T_Takeda_Ariba_Dump_Raw
	...
	update  T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta
		...
		dbo.Takeda_RemoveNonAlphaNonNumericCharacters([AllSupplier])


	update  T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta
		set [Company_LettersNumbersSpacesOnly] = 
		dbo.Takeda_RemoveNonAlphaNonNumNonSpace([AllSupplier])
	...
	update T_Takeda_Ariba_Dump_Raw_FormattedFields_Delta
		...
		/* and LEN(dbo.Takeda_RemoveNonAlphaNonNumericCharacters(SUBSTRING([AllSupplier],1,(CHARINDEX(' ',[AllSupplier] + ' ')-1)) )
		) >3 /* was 6 */ */
	...
	= LEN(replace(dbo.Takeda_RemoveNonAlphaNonNumNonSpace([AllSupplier]),'  ',' '))
		-LEN(dbo.Takeda_RemoveNonAlphaNonNumNonSpace([AllSupplier])) 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_JPS_DataLoad]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_0_JPS_DataLoad]

as

BEGIN

/* T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements */
/* company country added */

	drop table T_Takeda_ContractData_JPS_1VCOMPANY

	select * into T_Takeda_ContractData_JPS_1VCOMPANY
	FROM [dbo].[V_Takeda_ContractData_JPS_1VCOMPANY]
	...
	from
		T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements j
...
/*
			select * from T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements
			where companycountry is null */
GO
/****** Object:  StoredProcedure [dbo].[Takeda_0_LNC_DataLoad]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_0_LNC_DataLoad]

as

BEGIN

		delete from T_Takeda_KWS_0_Data_LINC 
			where [reference] in 
			(select [reference] from T_Takeda_KWS_0_Data_LINC_RAW)

/* RAW */
		INSERT INTO T_Takeda_KWS_0_Data_LINC 
		 ([Reference]
			  ,[Status]
			  ,[Contract type]
			  ,[Amendment no]
		...
		[Reference]
			  ...
			  ,GETDATE()
		FROM 
			T_Takeda_KWS_0_Data_LINC_RAW r
		where r.[reference] not in 
			(select [reference] from T_Takeda_KWS_0_Data_LINC)

	/* select *  into T_Takeda_KWS_0_Data_LINC_bak from T_Takeda_KWS_0_Data_LINC */
/* FINAL */
/*
	alter table [dbo].[T_Takeda_KWS_0_Data_LINC]
	alter column [Description] nvarchar(1500) 

	alter table [dbo].[T_Takeda_KWS_0_Data_LINC]
	add DateTableRefreshed datetime */

	update [dbo].[T_Takeda_KWS_0_Data_LINC]
	set DateTableRefreshed = Getdate() /* '20-Aug-2020' */ where DateTableRefreshed is null

	update T_Takeda_KWS_0_Data_LINC
	set [outside party] = ltrim([outside party]) /* spaces in CK dump for Nick */

	/* Final Table */
		drop table T_Takeda_ContractData_LNC_1VCOMPANY

			select * into T_Takeda_ContractData_LNC_1VCOMPANY
			FROM V_Takeda_ContractData_LNC_1VCOMPANY
/*   truncate table [dbo].[T_Takeda_KWS_0_Data_LINC_RAW] */

END

GO
/****** Object:  StoredProcedure [dbo].[Takeda_01_DWH_DataLoad_DRAFT]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_01_DWH_DataLoad_DRAFT]
...
as
	...
	Drop table T_Takeda_KWS_DWH_AllSystems_Union

	SELECT * INTO T_Takeda_KWS_DWH_AllSystems_Union
	FROM V_Takeda_KWS_DWH_AllSystems_Union

	/* index makes no sense on contractid since ariba has char ID */
	/* CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_DWH_AllSystems_Union_ContractID
	ON T_Takeda_KWS_DWH_AllSystems_Union (CONTRACTID, SourceSystem) */ 


/* 
bulk insert T_Takeda_KWSR_1_CNT_ARB
FROM 'M:\'
WITH (FORMAT = 'CSV') */

/* NO Permission for user contiki 
BULK INSERT T_Takeda_Ariba_Dump_Raw_FormattedFields_Staging
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_03DataLoad_KWS]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_03DataLoad_KWS]
...
BEGIN
	...
	/* Key Word Search Items */
		...
		/* T_TAKEDA_ALL_VIEW */
			drop table T_Takeda_KWS_0_ContikiView_CNT
	
			select * into T_Takeda_KWS_0_ContikiView_CNT
			from [V_Takeda_KWS_0_ContikiView_CNT]

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_0_ContikiView_CNT_CONTRACTID
			ON T_Takeda_KWS_0_ContikiView_CNT(CONTRACTID)

		/* PRODUCT */
			drop table T_Takeda_TPRODUCTGROUP
	
				select * into T_Takeda_TPRODUCTGROUP
				from [dbo].[V_Takeda_VPRODUCT]

				CREATE UNIQUE CLUSTERED INDEX T_Takeda_TPRODUCTGROUP_PRODUCTGROUPID
				ON T_Takeda_TPRODUCTGROUP (PRODUCTGROUPID) 

		/* COMPANY - also run daily! */

			drop table T_Takeda_VCOMPANY

				select * into T_Takeda_VCompany
				from [dbo].[V_Takeda_VCOMPANY]

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_0DataLoad]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_0DataLoad]

as

print 'this'
/*
/* t_takeda_Hierarchy run time on 6-feb-19 13:00 was 2 sec
, this must be run before T_Takeda_ALL because the hierarchy is used in V_Takeda_ALL */

	truncate table t_takeda_Hierarchy
	
		insert into T_Takeda_Hierarchy select *
		from [dbo].[V_Takeda_Hierarchy_MakeTable]	

		/* ONLY NEEDED AFTER INSERT CREATE UNIQUE CLUSTERED INDEX T_Takeda_Hierarchy_DEPARTMENTID
		ON T_Takeda_Hierarchy (DEPARTMENTID) */

		/* COMPANY - also run in KWS procedure */

/* update TCOMPANY run time 13 sec 23-feb */
	if OBJECT_ID('T_Takeda_VCOMPANY') is not null 
			drop table T_Takeda_VCOMPANY			

			select * into T_Takeda_VCompany
			from [dbo].[V_Takeda_VCOMPANY]

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_VCompany_COMPANYID
			ON T_Takeda_VCompany (COMPANYID_ln)
...
/* Tenderer FLAT */
	if OBJECT_ID('T_Takeda_TTENDERER_FLAT') is not null 
			drop table T_Takeda_TTENDERER_FLAT

			select * into T_Takeda_TTENDERER_FLAT
			FROM V_Takeda_TTENDERER_FLAT

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_TTENDERER_FLAT_CONTRACTID
			ON T_Takeda_TTENDERER_FLAT (CONTRACTID)

/* Dptroles FLAT - run time 4:41 */
	if OBJECT_ID('T_Takeda_VCONTRACT_DPTROLES_FLAT') is not null 
			drop table T_Takeda_VCONTRACT_DPTROLES_FLAT

			select * into T_Takeda_VCONTRACT_DPTROLES_FLAT
			FROM [dbo].[V_Takeda_VCONTRACT_DPTROLES_FLAT] 

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_VCONTRACT_DPTROLES_FLAT_DPT_CONTRACTID
			ON T_Takeda_VCONTRACT_DPTROLES_FLAT (DPT_CONTRACTID)

			update T_Takeda_VCONTRACT_DPTROLES_FLAT 
			...
			select * from T_Takeda_VCONTRACT_DPTROLES_FLAT 
...
/* V_Takeda_VPERSONROLE_IN_OBJECT */
	if OBJECT_ID('T_Takeda_VPERSONROLE_IN_OBJECT ') is not null 
			drop table T_Takeda_VPERSONROLE_IN_OBJECT 

			select * into T_Takeda_VPERSONROLE_IN_OBJECT 
			FROM [dbo].[V_Takeda_VPERSONROLE_IN_OBJECT ] 

			CREATE INDEX T_Takeda_VPERSONROLE_IN_OBJECT_Roleid_Cat2Letter
			ON T_Takeda_VPERSONROLE_IN_OBJECT (Roleid_Cat2Letter)

			CREATE INDEX T_Takeda_VPERSONROLE_IN_OBJECT_CONTRACTID
			ON T_Takeda_VPERSONROLE_IN_OBJECT (CONTRACTID)

		/*	update T_Takeda_VPERSONROLE_IN_OBJECT 
			set userid /* GGC sysadmin */
			where  Dpt_ContractOwnerDpt_ID IS NULL /* for hierarchy outer join */*/
/* [V_Takeda_VCONTRACT_PERSONROLES_FLAT]  */
	if OBJECT_ID('T_Takeda_VCONTRACT_PERSONROLES_FLAT') is not null 
			drop table T_Takeda_VCONTRACT_PERSONROLES_FLAT

			select * into T_Takeda_VCONTRACT_PERSONROLES_FLAT
			FROM [dbo].[V_Takeda_VCONTRACT_PERSONROLES_FLAT] 

			CREATE INDEX T_Takeda_VCONTRACT_PERSONROLES_FLAT_Prs_CONTRACTID
			ON T_Takeda_VCONTRACT_PERSONROLES_FLAT (Prs_CONTRACTID)

/* T_Takeda_ALL run time 21-Nov-18 was 9 minutes 
22-feb-21 - run time 2 min
*/
	if OBJECT_ID('T_Takeda_ALL') is not null 
		drop table T_Takeda_ALL

		select * into T_Takeda_ALL 
		from [dbo].[V_Takeda_ALL] 
		order by CONTRACTID asc

		CREATE UNIQUE CLUSTERED INDEX T_Takeda_ALL_CONTRACTID
		ON T_Takeda_ALL (CONTRACTID)

/* xt table run time 23-Feb 13s for 36k records*/

	if OBJECT_ID('T_Takeda_ALL_Xt') is not null 
		drop table T_Takeda_ALL_Xt

		select * into T_Takeda_ALL_Xt
		from [dbo].[V_T_Takeda_ALL_Xt] 
		order by CONTRACTID asc

		CREATE UNIQUE CLUSTERED INDEX T_Takeda_ALL_Xt_CONTRACTID
		ON T_Takeda_ALL_Xt (CONTRACTID)
	
/* T_Takeda_ALL */

	drop table T_Takeda_tdepartmentrole_in_object_GroupedByDptID

	select * into T_Takeda_tdepartmentrole_in_object_GroupedByDptID 
	from [dbo].[V_Takeda_tdepartmentrole_in_object_GroupedByDptID] 
	order by dpt_objectid asc

	CREATE INDEX T_Takeda_drio_GByDptIDCONTRACTID
	ON T_Takeda_tdepartmentrole_in_object_GroupedByDptID (Dpt_ObjectID)


*/

	if OBJECT_ID('T_Takeda_VPERSONROLE_IN_OBJECT ') is not null 
			drop table T_Takeda_LNC_GoldStandard_Documents

			select * into T_Takeda_LNC_GoldStandard_Documents
			FROM [dbo].[V_Takeda_LNC_GoldStandard_Documents] 

		CREATE INDEX T_Takeda_LNC_GoldStandard_Documents_DOCUMENTID
		ON T_Takeda_LNC_GoldStandard_Documents (DOCUMENTID)
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_0Edit_DataLoad]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_1DAILY_0Edit_DataLoad]
...
/* normal run time 00:10 */
	...
	drop table T_Takeda_Docx
	
	select * into T_Takeda_Docx 
	from [dbo].[V_Takeda_DocxValidSignedNotRegForm] 
	order by [DescRemNonAlphaHashbSHA1] asc

	CREATE INDEX T_Takeda_Docx_OBJECTID
	ON T_Takeda_Docx(OBJECTID) 
	
/* Duplicate Contracts */
/* normal run time 00:27 */

	/* [V_Takeda_Duplicates] is base duplicate detection query,
	 the view [V_Takeda_Duplicates_Final] includes both min and max to show them side by side in list 
	 while V_Takeda_EDIT_ITEMS only includes the min dupe number so that there is only one item showing
	 both duplicate #s */

	Drop table T_Takeda_Duplicates_Final

	select * into T_Takeda_Duplicates_Final 
	from [dbo].[V_Takeda_Duplicates_Final]
	order by CONTRACTID_UNIQUE

	CREATE INDEX T_Takeda_Duplicates_Final_CONTRACTID_DUPE
	ON T_Takeda_Duplicates_Final (CONTRACTID_UNIQUE)
	
	CREATE UNIQUE CLUSTERED INDEX T_Takeda_Duplicates_Final_CONTRACTID_UNIQUE
	ON T_Takeda_Duplicates_Final (CONTRACTID_UNIQUE)	
...
/* normal run time 01:19 */
	...
	drop table T_Takeda_EDIT_ITEMS
	
	select e.EditNo, a.* into  T_Takeda_EDIT_ITEMS 
	from V_Takeda_EDIT_ITEMS e 
		INNER JOIN V_T_Takeda_ALL_WeeklyEditRpt a 
	...
	CREATE INDEX T_Takeda_EDIT_ITEMS_CONTRACTID
	ON T_Takeda_EDIT_ITEMS(CONTRACTID) 
	


	/* 
	drop table T_Takeda_Edit_Wrong_DPTROLE_IN_OBJECT
	

	select * into T_Takeda_Edit_Wrong_DPTROLE_IN_OBJECT FROM
	V_Takeda_Edit_Wrong_DPTROLE_IN_OBJECT
	*/
	
/* V_Takeda_EditDocuments */
/* normal run time 01:34 */

	drop table T_Takeda_EditDocuments
	
	select * into  T_Takeda_EditDocuments 
	from V_Takeda_EditDocuments
	
	 CREATE UNIQUE INDEX T_Takeda_EditDocuments_DOCUMENTID
	ON T_Takeda_EditDocuments(DOCUMENTID)
	
	CREATE INDEX T_Takeda_EditDocuments_CONTRACTID
	ON T_Takeda_EditDocuments(CONTRACTID) 
	
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_ACL_AddMissingPermissions_AUTO]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[Takeda_1DAILY_ACL_AddMissingPermissions_AUTO]
AS
...
IF NOT EXISTS(select distinct r.contractid, u.usergroupid, u.privilege
	from V_Takeda_AUTO_ACL_ContractIDs  r /* excludes test delete case TS Confidential etc , ACL Table not used so OBJECTTYPEID not relevant */
		inner join dbo.V_Takeda_AUTO_ACL_Upload u  /* ACL Table not used so OBJECTTYPEID not relevant */
...
select distinct 
	...
	, u.usergroupid
	, u.privilege
from V_Takeda_AUTO_ACL_ContractIDs  r 
	inner join dbo.V_Takeda_AUTO_ACL_Upload u
on r.Code = u.Code3Digit
	inner join tcontract c on c.contractid = r.contractid
where r.contractid not in 
...
FETCH NEXT FROM myCursor INTO @OBJECTID, @groupid, @privilegeid
		/* PRINT 'Takeda_ACL_Upload_Group_ObjectIDGroupID'  
		PRINT @OBJECTID
		PRINT @groupid
		PRINT @privilegeid
		PRINT 0 */
WHILE @@FETCH_STATUS = 0 BEGIN
    ...
    WITH MARK N'ACL Permissions';
		...
		exec Takeda_ACL_Upload_Group_ObjectIDGroupID  @OBJECTID=@OBJECTID, @OBJECTTYPEID = 1, @GROUPID=@groupid, @PRIVILEGEID=@privilegeid, @NONINHERITABLE=0
	
		IF @privilegeid = 2 /*Write, implement Read also */
		BEGIN
			exec Takeda_ACL_Upload_Group_ObjectIDGroupID  @OBJECTID=@OBJECTID, @OBJECTTYPEID = 1, @GROUPID=@groupid, @PRIVILEGEID=1 /*Read*/, @NONINHERITABLE=0
		END 
...
PRINT '*** END [dbo].[Takeda_ACL_AddMissingPermissions]'


END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_ACL_AddRemoveAutoPermissions]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_ACL_AddRemoveAutoPermissions]
...
/* 1a READ Permission*/
	...
	select distinct 
		...
		, 1 /*PRIVILEGE READ*/
		, 0 /*inheritable*/
	from V_Takeda_TCONTRACT_ACL_Auto_Excl_TstDelMig_TSConfidential
	where 
		contractid not in (select ta.objectid from tacl ta
			where 
			ta.objecttypeid = 1 /*contract*/ and 
	...
	AND [AGREEMENT_TYPEID] not in (select AgrTypeID from V_Takeda_AgreementType
...
/* 2a Top Secret READ Permission*/
	...
	select distinct 
		...
		, 1 /*PRIVILEGE READ*/
		, 0 /*inheritable*/
	from  V_Takeda_TCONTRACT_ACL_Auto_Excl_TstDelMig
	where 
	contractid not in (select ta.objectid from tacl ta
						where 
						ta.objecttypeid = 1 /*contract*/ and 
	...
	AND ACLID IN
			...
			AND 
				(OBJECTID not IN (SELECT CONTRACTID 
									FROM [dbo].[V_Takeda_TCONTRACT_ACL_Auto_Excl_TstDelMig_TSConfidential]	
									WHERE 
										contracttypeid <> '11' /* exclude CASE */
									AND [AGREEMENT_TYPEID] not in (select AgrTypeID 
											from V_Takeda_AgreementType
											WHERE [AgreementType_IsPrivate_FLAG] = 0 /* public */ 
...
/* 9 Ariba migration records - delete write permission - move out of this procedure to separate Ariba adhoc */
	...
	select distinct 
		...
		, 3 /*PRIVILEGE CREATE*/
		, 0 /*inheritable*/
	from V_Takeda_TCONTRACT_ACL_Auto_Excl_TstDelMig c /* WITH CONFIDENTIAL */
	where 
		contractid not in 
...
/*	don't run on a weekend for first time
	...
	and u.userid not in (select r.userid from [dbo].[V_Takeda_VPPERSONROLE_IN_OBJECT] r 
...
/* missing CREATE Privilege on documents */
	...
	from V_Takeda_TCONTRACT_ACL_Auto_Excl_TstDelMig c 
		inner join V_takeda_vdocument d on c.CONTRACTID = d.contractid /* WITH CONFIDENTIAL */
...
/* see comment - non issue
	...
	from v_takeda_vdocument d inner join TCONTRACT c on d.contractid = c.contractid
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_AUTODELETE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_AUTODELETE]
...
/*7 DELETE, !AUTODELETE */
		...
		*/
		...
		/* now directly in daily pro 
			exec Takeda_1DAILY_AUTODELETE_Deactivate_Reminders		
				/* Delete all permissions for num series 106 AUTODELETE except admin, delete etc. */
			exec Takeda_1DAILY_AUTODELETE_RemovePermissions */
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_AUTODELETE_Deactivate_Reminders]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_AUTODELETE_Deactivate_Reminders]
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_AUTODELETE_RemovePermissions]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_AUTODELETE_RemovePermissions]
AS
...
IF not EXISTS(select ACLID 
	...
	WHERE 	
		...
		AND OBJECTID IN
			...
			AND  (USERID not in (1 /*sysadm */, 20134 /* contikiadmin */, 81995 /* systemservice */) or USERID is null)
			)
...
WHILE @@FETCH_STATUS = 0 BEGIN
    ...
    WITH MARK N'ACL Permissions';
		...
		exec Takeda_ACL_Remove_ACLID  @ACLID, 1 /* @OBJECTTYPEID Contract */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_ConfidentialityFlag]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_1DAILY_ConfidentialityFlag]
...
/* TOP SECRET - INSERT */
  ...
  INSERT INTO [Contiki_app].[dbo].[TEXTRA_FIELD_IN_CONTRACT]
			( 
...
/* Confidentiality Flags, [EXTRA_FIELDID] = 100002 */
  ...
  UPDATE  [Contiki_app].[dbo].[TEXTRA_FIELD_IN_CONTRACT] 
...
/* Material Agreement Flag, [EXTRA_FIELDID] = 100003 */
  ...
  SET [MIK_EDIT_VALUE] = 'Material' /* Top Secret etc. */
	...
	AND CONTRACTID IN 
		 ( SELECT CONTRACTID
	  FROM T_Takeda_ALL /* not TCONTRACT because lump sum needed */
	  WHERE 
...
*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1DAILY_PUBLIC_READ_Permissions]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1DAILY_PUBLIC_READ_Permissions]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1HOURLY_CancelCheckoutAllPdfs]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1HOURLY_CancelCheckoutAllPdfs]

as

/* #410 - Cancel checkout all PDFs '*/
/* analogue to 'Cancel checkout on all documents' script in Contiki install package */
BEGIN
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_1HOURLY_SetToAwarded_DeleteAwardedDate]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_1HOURLY_SetToAwarded_DeleteAwardedDate]
...
BEGIN
	...
	/* notes: if awarded date is null, still do this? , refer to [dbo].[Takeda_CORRECTION_STATUS] */
	update [TTENDERER]
		set [ISAWARDED] =  1 /* Awarded */
		, comment = 'set to awarded ' +CONVERT(CHAR(10),  GETDATE(), 120) 
	where [ISAWARDED] = 0 /* NOT Awarded */

	PRINT '[Takeda_409_SetContractsToAwarded]Result: ' + Convert(Varchar(12),@@ROWCOUNT) + ' Rows Updated'

	/* 8. Delete Awarded Dates so that other dates can be changed without errors */

		PRINT '8. Delete Awarded Dates'

/* removed since it is causing status issues, refer to [dbo].[Takeda_CORRECTION_STATUS] */
	/* UPDATE tCONTRACT
	SET [AWARDDATE] = NULL
	WHERE [AWARDDATE] IS NOT NULL */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2DAILY_AlphaSort_TDEPARTMENT]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_2DAILY_AlphaSort_TDEPARTMENT]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2DAILY_UpdateStrategyTypeTakPhVertrieb]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_2DAILY_UpdateStrategyTypeTakPhVertrieb]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_CompanyCleanup_TakedaIntercompany]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_2WEEKLY_CompanyCleanup_TakedaIntercompany]
...
PRINT '[dbo].[Takeda_341_RemapTT_InternalPartnerToTerritory]'



/* Merge junk Takeda entities into Intercompany Dummy entity */
	UPDATE TTENDERER
	SET COMPANYID = 1 /* Intercompany Takeda */
	WHERE 
		COMPANYID <> 1 /* not already Intercompany Takeda */
		and COMPANYID <> 224910 /* INTRAcompany Takeda */
		AND COMPANYID IN (SELECT COMPANYID
							...
							or company like '%Takeda%')
							)


	PRINT 'Result Flag Merge into Intercompany Takeda: ' + Convert(Varchar(12),@@ROWCOUNT) + ' Rows Affected'

/* Delete Primary Contact ID if there is one */
	UPDATE TTENDERER
	SET PRIMARYCOMPANYCONTACTID = NULL 
	WHERE 
	COMPANYID = 1 /* Intercompany Takeda */
...
/* Disable companies without contracts attached */
	...
	WHERE 
		MIK_VALID = 1
		AND COMPANYID <> 1 /* not Intercompany Takeda */
		AND COMPANYID <> 224910 /* not INTRAcompany Takeda */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_HardcopyArchiving_DeleteTTEntries]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[Takeda_2WEEKLY_HardcopyArchiving_DeleteTTEntries]
...
PRINT '[Takeda_361_Weekly_HardcopyArchiving_DeleteTTEntries] Result: ' + Convert(Varchar(12),@@ROWCOUNT) + ' Rows Deleted'


END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_Maintenance_AgreementTypes]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_2WEEKLY_Maintenance_AgreementTypes]

AS

BEGIN

	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'[Takeda_Maintenance_AgreementTypes] START')
	...
	/* Contract Relation - MSA */
		...
		and AGREEMENT_TYPEID in (select AgrTypeID from V_Takeda_AgreementType
	...
	/*	update tcontract
		...
		*/
		...
		set STRATEGYTYPEID = 21 /* HCP HCO */
			...
			and contractid in (select contractid from V_Takeda_Departmentrole_In_Object 
				where roleid = 2 and department_code = '.DES*' /* Tak DE Vertrieb */)
			and [contract] not like 'ARIBA%'
			and contract not like '!AUTODELET%'
			and contractid in (select contractid from V_Takeda_TTenderer_Tcompany 
...
/* HCP/HCO flags, must be done AFTER any agreement type conversions above */
	...
	/* T_Takeda_AgreementType - Agreement Type Material Flag */
		insert into 
			[dbo].[T_Takeda_AgreementType] 
		select 1 /* material flag */ 
		 ...
		 where AGREEMENT_TYPEID not in (select agr_typeid 
			from [dbo].[T_Takeda_AgreementType])

		update a
		set [AgrName] = ag.AGREEMENT_TYPE
		from 
			[T_Takeda_AgreementType] a inner join TAGREEMENT_TYPE ag 
			on a.Agr_typeid = ag.AGREEMENT_TYPEID
		where 
			[AgrName] is null 
			OR [AgrName] <> ag.AGREEMENT_TYPE

	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'[Takeda_Maintenance_AgreementTypes] END')

END

GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_RemapTT_IPToTT]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_2WEEKLY_RemapTT_IPToTT]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_RenameDTitleToCTitle]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [dbo].[Takeda_2WEEKLY_RenameDTitleToCTitle]
...
FROM
    tdocument d
INNER JOIN
    [V_Takeda_Edit_DocAutoRenameCTitle] c
ON 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_2WEEKLY_ReplaceUnderscoresWithDashes]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_2WEEKLY_ReplaceUnderscoresWithDashes]
...
PRINT '[dbo].[Takeda_368_ReplaceUnderscoresWithDashes]'
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_AddCurrentIP_ForActiveContracts]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_AddCurrentIP_ForActiveContracts]
...
INSERT INTO TDEPARTMENTROLE_IN_OBJECT (OBJECTTYPEID, OBJECTID, ROLEID, DEPARTMENTID ) 
	...
	FROM TDEPARTMENTROLE_IN_OBJECT r
		inner join  [V_Takeda_VDepartment_ParsedDpt_InternalPartner] d1 
...
/*	INSERT INTO TDEPARTMENTROLE_IN_OBJECT (OBJECTTYPEID, OBJECTID, ROLEID, DEPARTMENTID )
	...
	WHERE contractid not in (select objectid from 
			V_takeda_vdepartmentrole_in_object 
			where ROLEID IN(0,6/*ENTITY*/,100 /*INTERNAL PARTNER*/))
	and contractid in (select contractid from [V_Takeda_LNC_GoldStandard])
*/
/*
	select * from t_takeda_all where internalpartners =''
	update T_Takeda_ALL_Xt
		set [InternalPartners_IDs] = '203266' 
		, [InternalPartners] = '(Switzerland) Takeda Pharmaceuticals International GmbH'
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_AddDocumentTag_FullText]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_AddDocumentTag_FullText]
AS
/* Notes:
...
where tagid in (select tagid from T_Takeda_Tag_in_Document)
*/
...
/*dbo.T_Takeda_Tag_in_Document
 [V_Takeda_TTag_Summary] */
/*
select sum([Count])
  FROM [Contiki_app].[dbo].[V_Takeda_TTag_Summary]
  */

	DECLARE curLabels CURSOR LOCAL FAST_FORWARD FOR

	select TagID, Keyword from [dbo].[T_Takeda_Tag_in_Document] /* Aliases, language equivalents, thesaurus */
	...
	PRINT 'TAGID' + str(@TAGID)
						...
						EXEC [dbo].[Takeda_TagUpload_DocumentID] 
							@TAGID
...
/* delete from T_Takeda_Product_Upload */
	CLOSE curLabels
...
END 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_CorrectContractStatus]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_CorrectContractStatus]

AS 

BEGIN

/* please also see  [dbo].[Takeda_Maintenance_ContractStatus_CheckAndCorrection] */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_FlagBlank_CountryCityStreet]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_FlagBlank_CountryCityStreet]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_FlagNoFiles]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_FlagNoFiles]
...
/* 6 '*NO FILES*' Flag SEE Takeda_319_FlagNoFiles */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_ARB_Description]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_ARB_Description]
AS

/* run time on 23-Apr was 3 hours 14 min - worth streamlining like Contiki script? */
...
from V_Takeda_VPRODUCTGROUP /* p  inner join T_Takeda_KeyWordSearch k on p.PRODUCTGROUP = k.KeyWordVarchar255 */
...
/************************** CHECK IF ANY RECORDS */
		 IF EXISTS (
			...
			AND ContractNumber NOT IN (SELECT ContractNumber
					from T_Takeda_Ariba_Products_In_Contracts
...
/*!!! Exact Match Level = 1 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
			...
			WHERE (c.[Contract Description] like @PRODUCTGROUP_LEFTBLANK 
				...
				WHILE @@FETCH_STATUS = 0 BEGIN
					...
					PRINT @DESCRIPTION
						EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID_ARIBA @CONTRACTNUMBER,  @CONTRACTINTERNALID, @PRODUCTGROUPID, @SOURCENAME,@MATCHLEVEL
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_CNT_Description]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_CNT_Description]
AS
...
/* WHEN NEW PRODUCTS ARE ADDED, or old ones modified, run the procedure WITHOUT the date filter  to make sure the product is tagged in older records too */
	...
	WHILE @@FETCH_STATUS = 0 BEGIN
			...
			IF EXISTS (
				...
				AND (c.CONTRACTID in (select objectid from V_Takeda_Audittrail_ModLast30DaysMin1DayOld where OBJECTTYPEID = 1) 
					OR c.CONTRACTID in (select objectid from tdocument 
							where documentid in (select objectid 
								from V_Takeda_Audittrail_ModLast30DaysMin1DayOld where OBJECTTYPEID = 7 /* document */ ) 
										)
					...
					/* contracts cursor */
						...
						/* Fetch loop while there are records */
							WHILE @@FETCH_STATUS = 0 BEGIN
								...
								PRINT 'PRODUCTGROUP: ' + @PRODUCTGROUP + ' - (Takeda_2WEEKLY_ProductGroupUpload_Description)'
								PRINT @CONTRACTNUMBER
								PRINT @DESCRIPTION

									EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID 
									@OBJECTID
									, @PRODUCTGROUPID
									, 1 /* OBJECTTYPEID */
									, @PRODUCTGROUP
			...
			ELSE 
				...
				IF @DEBUG_OUTPUT = 1 
				PRINT '  No (new) records for : '  + @PRODUCTGROUP +' (Takeda_2WEEKLY_ProductGroupUpload_Description)';
	...
	END
		...
		CLOSE curProducts
		DEALLOCATE curProducts
		SET @RESULTSTRING = '  Successfully completed, END CurProducts (Takeda_2WEEKLY_ProductGroupUpload_Description)'

		GOTO lblEnd

	lblTerminate: 
		PRINT 'lblTerminate: !!! Statement did not execute due to invalid input values! (Takeda_2WEEKLY_ProductGroupUpload_Description)'

	lblEnd: 
		
		/* Archive upload table records older than 14 days */
		EXEC [dbo].[Takeda_ProductGroupUpload_ArchiveLogTable]

		PRINT @RESULTSTRING

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_CNT_FullText]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_CNT_FullText]
AS
/* Notes:
Currently excluding lots of names with special chars to be sure
retest
		...
		PRINT 'Product Group: '  + @PRODUCTGROUP + ', ID: ' + convert(varchar(10),@PRODUCTGROUPID)
			...
			BEGIN 
				...
				BEGIN
						...
						/*   Takeda_ProductGroupUpload_ObjectidProductgroupID
									  @OBJECTID bigint 
					...
					*/
					EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID @OBJECTID, @PRODUCTGROUPID, @OBJECTTYPEID, @PRODUCTGROUP, '' /*@DESCRIPTION */, @CONTRACTNUMBER, @DATEREGISTERED
					
					INSERT INTO T_Takeda_Product_Upload ( 
						PRODUCTGROUPID       
					   , OBJECTID 
					   , OBJECTTYPEID
...
END 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_JPS_Description]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_PrdGrpUpload_JPS_Description]
AS
...
/*!!! Exact Match Level = 1 * !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
				...
				SELECT @PRODUCTGROUPID AS PRD 
					...
					/* , max(c.[Begin Date]) as [Begin Date]
					, c.[Contract Description] */
				FROM [dbo].[T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements] c 
			WHERE (c.[ProductClean] like @PRODUCTGROUP_LEFTBLANK 
				...
				AND ContractID NOT IN (SELECT ContractID
						from T_Takeda_ContractData_JPSunrise_Products_In_Contracts
						WHERE PRODUCTGROUPID = @PRODUCTGROUPID)													
				...
				WHILE @@FETCH_STATUS = 0 BEGIN
					...
					PRINT @DESCRIPTION*/
						...
						EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID_JPSunrise @CONTRACTNUMBER,  @CONTRACTID, @PRODUCTGROUPID, @SOURCENAME,@MATCHLEVEL
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_RemapTT_AlphaToRegion]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_3SATNIGHT_RemapTT_AlphaToRegion]
...
/* update TDEPARTMENTROLE_IN_OBJECT with mapping table */
	...
	PRINT '[dbo].[Takeda_327_RemapTT_AlphaToRegion] Result: ' + Convert(Varchar(12),@@ROWCOUNT) + ' Rows Affected'
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_3SATNIGHT_Reminders_Deactivate_ExpIntercompany]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_3SATNIGHT_Reminders_Deactivate_ExpIntercompany]
...
/* Expired contracts */
	/* TWARNING , review date only (maybe extend to all) */
		...
		/* AND objectid = 133810 /* 'Contract-11127305' */ */
		AND objectid in (select contractid from T_Takeda_ALL c 
	...
	/* tperson_in_warning */
		...
		,isturnedoff = 1 /* OFF */
		where (tperson_in_warning.isturnedoff = 0 /* ON */ or TURNEDOFFDATE is null)
		and warningid in (select warningid from twarning w inner join T_Takeda_ALL c on c.contractid = w.objectid
...
/* ENTER MIGRATION DATE before running */
		...
		where warningid in (select warningid from twarning w inner join T_Takeda_ALL c on c.contractid = w.objectid
						where c.COUNTERPARTYNUMBER = '!ARIBA_W01')				

*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_489_NoReviewDateReminder]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_489_NoReviewDateReminder]
...
/* DEACTIVATED???? */
	...
	from TCONTRACT c inner join V_Takeda_ALL a on c.CONTRACTID = A.CONTRACTID
	where
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_491_SignedContractsInCase]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_491_SignedContractsInCase]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_4MONTHLY_AlphaSort_AGREEMENT_TYPE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_4MONTHLY_AlphaSort_AGREEMENT_TYPE] 

AS

BEGIN
 
/* Takeda Issue # 333 - Alphabetical Sort Order TUSERGROUP */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_4MONTHLY_AlphaSort_TUSERGROUP]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_4MONTHLY_AlphaSort_TUSERGROUP] 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_4MONTHLY_DeleteDocsRecycleBin]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/* CMA Contiki Performance Pack
Delete documents from rec bin
Ready for  6.10.5.x DB
Ready for  6.11.x   DB
*/

CREATE PROCEDURE [dbo].[Takeda_4MONTHLY_DeleteDocsRecycleBin]
/* added to MONTHLY script */
as
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ACL_Remove_ACLID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE  PROCEDURE [dbo].[Takeda_ACL_Remove_ACLID](
                @ACLID bigint
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ACL_Upload_Group_ObjectIDGroupID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE  PROCEDURE [dbo].[Takeda_ACL_Upload_Group_ObjectIDGroupID](
                @OBJECTID bigint 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ACL_zPerContractid_AllPermissions]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_ACL_zPerContractid_AllPermissions](
                @OBJECTID bigint 
...
from [Contiki_app].[dbo].[VCONTRACT_DEPARTMENTROLES]  r inner join dbo.V_Takeda_AUTO_ACL_Upload u
...
WHILE @@FETCH_STATUS = 0 BEGIN
    ...
    exec Takeda_ACL_Upload_Group_ObjectIDGroupID  @OBJECTID=@OBJECTID, @GROUPID=@groupid, @PRIVILEGEID=@privilegeid, @NONINHERITABLE=0
	
	IF @privilegeid = 2 /*Write, implement Read also */
	BEGIN
	    exec Takeda_ACL_Upload_Group_ObjectIDGroupID  @OBJECTID=@OBJECTID, @GROUPID=@groupid, @PRIVILEGEID=1, @NONINHERITABLE=0
	END 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Adhoc_EditContractRelation]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_Adhoc_EditContractRelation]
...
*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Adhoc_FullText]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_Adhoc_FullText]
AS
...
END 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Adhoc_Remap_TT_IP]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Adhoc_Remap_TT_IP]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Adhoc_VariousSingleUpdates]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Adhoc_VariousSingleUpdates]
...
/* update super user in contract */
	...
	where OBJECTID in (select contractid from T_Takeda_ALL 
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Ariba_ProductUploadFullText]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [dbo].[Takeda_Ariba_ProductUploadFullText]
...
from [dbo].[T_Takeda_Ariba_ProductsFullText_Upload] u inner join T_Takeda_AribaDump d on u.contractnumber = d.ContractNumber

update u 
set u.productgroupid = p.productgroupid
from [dbo].[T_Takeda_Ariba_ProductsFullText_Upload] u inner join TPRODUCTGROUP p on u.productname = p.PRODUCTGROUP

update [T_Takeda_Ariba_ProductsFullText_Upload] set source = 'FullText'

INSERT INTO T_Takeda_Ariba_Products_In_Contracts_FullText 
([ContractNumber]
           ,[Product]
...
FROM [dbo].[T_Takeda_Ariba_ProductsFullText_Upload]
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ConfidentialityFlag_Insert]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_ConfidentialityFlag_Insert]

as

BEGIN
select top 1 1 from t_takeda_all
/* TOP SECRET - INSERT */
/* now replaced by Takeda_1DAILY_ConfidentialityFlag */
/*
  INSERT INTO [Contiki_app].[dbo].[TEXTRA_FIELD_IN_CONTRACT]
			( 
		  [EXTRA_FIELDID] /* 100002 = Confidentiality Flag */
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Ariba_ContractID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Ariba_ContractID](
                  @CONTRACTID bigint
...
/* NOTE: it could be that these are automatically deactivated once an end date is set - check on wave 3 first */
			EXEC [dbo].[Takeda_Reminders_Deactivate_ContractID] @CONTRACTID , @ExpiryDate /* expiry date is needed for TPerson turnedoffdate field */
		 ...
		 /* unique index is on ACLID to prevent dupes */
			...
			PRINT '!!! Takeda_Reminders_Deactivate_ContractID: '
			EXEC Takeda_Reminders_Deactivate_ContractID @CONTRACTID, @ExpiryDate
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Ariba_ContractID_REVERSE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Ariba_ContractID_REVERSE](
                @CONTRACTID bigint
...
2. Run this procedure: exec Takeda_Convert_Ariba_ContractID_REVERSE CONTRACTID,'ARIBA_W02_EXCEPT' 
contract is initially read only
remove CW2461283 from title
3. run Takeda_Maintenance_AddRemoveAutoPermissions manually to add permissions for legal back
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_ARIBA_Cursor]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_ARIBA_Cursor] 
...
/*
	...
	select * into T_Takeda_Xt_Detail 
	from TCONTRACT
	where COUNTERPARTYNUMBER = '!ARIBA_Wx'
	*/

/* back up acl for T_Takeda_Adhoc_ContractNumberFileDownload records first in case of restores / reverse mig

	select * into T_Takeda_AribaW2_ACL 
	from tacl a
	inner join T_Takeda_Adhoc_ContractNumberFileDownload l on a.objectid = l.contractid

		select * into T_Takeda_AribaW2_ACL_Deletes
	from TACL 
	WHERE 	
		PRIVILEGEID = 2 /* WRITE */
		AND OBJECTID IN (SELECT CONTRACTID FROM TCONTRACT WHERE COUNTERPARTYNUMBER like '!ARIBA%' /* migrated records */)
	...
	where 
	contractid in (select contractid from T_Takeda_Adhoc_ContractNumberFileDownload)
	...
	where CONTRACTID in (select contractid from T_Takeda_Adhoc_ContractNumberFileDownload)
...
/* Notes: counterparty field must have Ariba wave # in it or script won't run */
	...
	/*delete from T_Takeda_Adhoc_ContractNumber_AribaMigration where counterpartynumber = '!ARIBA_W02'*/

	update m
	set m.CONTRACTID = t.contractid
	from T_Takeda_Adhoc_ContractNumber_AribaMigration m 
	...
	FROM    dbo.T_Takeda_Adhoc_ContractNumber_AribaMigration d 
	...
	FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix,@CONTRACTINTERNALID,  @ExpiryDate, @CommentPrefix
		...
		WHILE @@FETCH_STATUS = 0 BEGIN
			...
			EXEC Takeda_Convert_Ariba_ContractID @CONTRACTID, @XtPrefix, @CONTRACTINTERNALID, @ExpiryDate, @CommentPrefix
			FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix,@CONTRACTINTERNALID,  @ExpiryDate, @CommentPrefix
		END		 
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Xt_ContractID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Xt_ContractID](
                @CONTRACTID bigint
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Xt_ContractID_REVERSE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Xt_ContractID_REVERSE](
                @CONTRACTID bigint 
                , @XtPrefix as VARCHAR(255)
                /*, @ExpiryDate as date */
)
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Xt_Cursor]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Xt_Cursor]
...
BEGIN		
	...
	FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix, @ExpiryDate
		...
		WHILE @@FETCH_STATUS = 0 BEGIN
			...
			EXEC Takeda_Convert_Xt_ContractID @CONTRACTID, @XtPrefix, @ExpiryDate
			FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix, @ExpiryDate
		END		 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Xt_Cursor_FlagContracts]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

create PROCEDURE [dbo].[Takeda_Convert_Xt_Cursor_FlagContracts]
...
begin
	...
	from [T_Takeda_XtConvert] xt
	where contractid is null

	select c.contractnumber, c.COUNTERPARTYNUMBER from TCONTRACT c inner join [T_Takeda_XtConvert] xt on c.CONTRACTID = xt.contractid
	...
	from TCONTRACT c inner join [T_Takeda_XtConvert] xt on c.CONTRACTID = xt.contractid
	WHERE 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Convert_Xt_Cursor_REVERSE]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Convert_Xt_Cursor_REVERSE]
...
/* Counter Party Number must match the @XtPrefix */
	IF NOT EXISTS ( 	SELECT  1  /* , EXPIRYDATE, rev_expirydate, DEFINEDENDDATE, statusid, reviewdate */
		FROM    dbo.tcontract c inner join dbo.T_Takeda_Xt x 
...
BEGIN		
	...
	FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix
		...
		WHILE @@FETCH_STATUS = 0 BEGIN
			...
			EXEC Takeda_Convert_Xt_ContractID_REVERSE @CONTRACTID, @XtPrefix
			FETCH NEXT FROM curContracts INTO @CONTRACTID, @XtPrefix
		END		 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_CORRECTION_433_Duplicates_tdepartmentrole_in_object]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_CORRECTION_433_Duplicates_tdepartmentrole_in_object]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_FileDownload_DONOTUSE_AdhocContractList]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_FileDownload_DONOTUSE_AdhocContractList]
...
where fileid in (select fileid from V_Takeda_VDOCUMENT where contractid in
	(SELECT CONTRACTID FROM T_Takeda_Adhoc_ContractNumberFileDownload )
	)
and FileType in( '.pdf', '.doc', '.docx', '.txt', '.pptx', '.msg')
	...
	AND fileid in (select fileid from V_Takeda_VDOCUMENT where contractid in
	(SELECT CONTRACTID FROM T_Takeda_Adhoc_ContractNumberFileDownload )
	)
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_FullText]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_FullText]
...
BEGIN
	...
	/* see Adhoc_FullText */
	/* T_Takeda_Tag_in_Document */
	/* Takeda_3SATNIGHT_AddDocumentTag_FullText */
	/* documents currently tagged but not CONTRACTS, however, T-Takeda_ALL has Tags field that concatenates doc tags */
	/* add log table */
	/* views such as ..*/
	select * from V_Takeda_FullText_ChangeOfControl

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_FullText_ProcessSteps]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_FullText_ProcessSteps]
...
table in Access Adhoc db is T_Takeda_Adhoc_ContractNumberFileDownload 
use file dsn if CONTIKI_PROD DSN does not work */

/* update fetch counter */
EXEC Takeda_FullText_TFILEINFO_OCRFetchCounter

/* get overview */
	select top 1000 * 
	from V_Takeda_FullText_TFILEINFO_OCR_FileIDsInScope /* more than 7 days old, ocr flag is null or 0 - no keywords found including Docusign etc */
	where LastChangedBy <>83663 /* not already full text scanned */
		
	/* in access use file dsn to upload table with user Contiki if sso does not work */
...
/* if you only want signed contracts: add WHERE d.DOCUMENTTYPEID = 1 *//*'Signed Contracts'*/
	...
	fileid in (select top 1000 fileid 
				from V_Takeda_FullText_TFILEINFO_OCR_FileIDsInScope /* fetch counter = 0 */ 
				where filetype = '.pdf'
		) 
	AND FileType = '.pdf'
...
/* RUN bat cmdRenamePdf1.bat (M:\Contiki_Confidential\Maintenance\OCR_Scan) */
...
/* MAKE BACKUP OF ORIGINALS TO ADMIN FOLDER */
	...
	/* when done check result folder to see if keyword Takeda can be found in files 
	Explorer search for content:docusign, contains many from Karin that are mixed signature
	when scan all done look for keyword Takeda and move all files where keyword found, the others are not scanned
	*/

	/*
	check for Dcousign files with mixed or full Dcousign signature but not readable /* 
	exec [dbo].[Takeda_TagUpload_DocumentID]  34 /* Docusign */ ,7 /* Document */, 111111 /* must convert to DocumentID */ 
	exec [dbo].[Takeda_TagUpload_DocumentID]  21 /* OCR Bad Scan */, 7 , /* must convert to DocumentID */ 
	(select documentid from VDOCUMENT where FileID = 376450) 
	376450, 406948

		exec [dbo].[Takeda_TagUpload_DocumentID]  21 /* OCR Bad Scan */, 7 , 365412
		exec [dbo].[Takeda_TagUpload_DocumentID]  21 /* OCR Bad Scan */, 7 , 395906

	*/
	use workbook in \\DES80022.nycomed.local\fs10$\Shares\AA-Data-Legal-Transfer\Contiki_Confidential\Maintenance\OCR_Scan
	to tag

	select DOCUMENTID, FileName, Owner, versiondate from [dbo].[T_Takeda_EDIT_DocusignFullTextScannedFiles] e inner join VDOCUMENT d on e.file_name = d.FileID


	update [dbo].[T_Takeda_EDIT_DocusignFullTextScannedFiles]
set id = fl

select file_name, owner, max(filename) as SampleFileName, COUNT(*) from V_Takeda_EDIT_DocusignFullTextScannedFiles
group by owner
...
EXEC [dbo].[Takeda_FullText_TFILEINFO_OCRFetchCounter] 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_FullText_TFILEINFO_OCRFetchCounter]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_FullText_TFILEINFO_OCRFetchCounter]
/* 
instructions: \\DES80021.nycomed.local\fs14$\Shares\DEKON23\Rechtsabteilung\Contiki\Regular Tasks\OCR Scan
...
/* find files that are already full text scanned */
	...
	where 
		...
		and fileid in  (SELECT KEY_TBL.[KEY] 
								...
								AS KEY_TBL WHERE KEY_TBL.RANK > 1) /* takeda France files */
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KeyWord_Compare]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KeyWord_Compare]
...
BEGIN
 ...
 /* 0 */
		drop table T_Takeda_VCompare_Results_0Exact
	
		select * into T_Takeda_VCompare_Results_0Exact
		from [V_Takeda_VCompare_Results_0Exact]

	/* 1 */

		drop table T_Takeda_VCompare_Results_1LikeFull
	
		select * into T_Takeda_VCompare_Results_1LikeFull
		from [dbo].[V_Takeda_VCompare_Results_1LikeFull]

	/* 2 */

		drop table T_Takeda_VCompare_Results_2FirstWord
	
		select * into T_Takeda_VCompare_Results_2FirstWord
		from [dbo].[V_Takeda_VCompare_Results_2FirstWord]

	/* 3 */
		drop table T_Takeda_VCompare_Results_3LikeLeft8
	
		select * into T_Takeda_VCompare_Results_3LikeLeft8
		from [dbo].[V_Takeda_VCompare_Results_3LikeLeft8]

		/* CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWSR_0_CNT_ARB_CONTRACTID
		ON T_Takeda_KWSR_1_CNT_ARB (CONTRACTID) */


END

GO
/****** Object:  StoredProcedure [dbo].[Takeda_KeyWordSearch]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KeyWordSearch]
...
/* [dbo].[V_T_Takeda_ALL_KeyWordSearch] */
...
/* Ariba: [dbo].[Takeda_Ariba_ProductUploadFullText] */
...
/* Keyword Search Table */
	...
	update T_Takeda_KeyWordSearch_Input
	...
	drop table T_Takeda_KeyWordSearch

		select * into T_Takeda_KeyWordSearch
		from [dbo].[V_Takeda_KWS_02_Input_AllFields] /* source table T_Takeda_KeyWordSearch_Input */

/*			update T_Takeda_KeyWordSearch
		set [KeyWordVarchar255] = dbo.Takeda_RemoveNonAlphaNonNumNonSpaceNonHyphen([KeyWordVarchar255])
*/
	/* strip last char if it is a special char */
/*	now done via strip function - update T_Takeda_KeyWordSearch
...
/**********************************/	
	/* 2 - Contiki */
		...
		drop table T_Takeda_KWS_2_CNT_TPRODUCT_ContractID

			select * into T_Takeda_KWS_2_CNT_TPRODUCT_ContractID
			from dbo.[V_Takeda_KWS_2_CNT_TPRODUCT_ContractID]

		drop table T_Takeda_KWS_2_CNT_TCompany_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_CNT_TCompany_ContractID
			from [dbo].[V_Takeda_KWS_2_CNT_TCompany_ContractID] 

		drop table T_Takeda_KWS_2_CNT_TCOMPANYCountry_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_CNT_TCOMPANYCountry_ContractID
			from [dbo].[V_Takeda_KWS_2_CNT_TCOMPANYCountry_ContractID] 

		drop table T_Takeda_KWS_2_CNT_InternalPartner_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_CNT_InternalPartner_ContractID
			from [dbo].[V_Takeda_KWS_2_CNT_InternalPartner_ContractID] 

		/*	CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_2_CNT_InternalPartner_ContractID
			ON T_Takeda_KWS_2_CNT_InternalPartner_ContractID (CONTRACTID) */
	
		/* 2 - Contiki - Tag */
		drop table T_Takeda_KWS_2_CNT_Tag_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_CNT_Tag_ContractID
			from [dbo].[V_Takeda_KWS_2_CNT_Tag_ContractID] 

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_2_CNT_Tag_ContractID
			ON T_Takeda_KWS_2_CNT_Tag_ContractID (CONTRACTID)
			

		drop table T_Takeda_KWS_2_CNT_Territories_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_CNT_Territories_ContractID
			from [dbo].[V_Takeda_KWS_2_CNT_Territories_ContractID] 

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_2_CNT_Territories_ContractID
			ON T_Takeda_KWS_2_CNT_Territories_ContractID (CONTRACTID)		
				
	/* 2- Ariba */
		drop table T_Takeda_KWS_2_ARB_TPRODUCT_ContractID

			select * into T_Takeda_KWS_2_ARB_TPRODUCT_ContractID
			from V_Takeda_KWS_2_ARB_TPRODUCT_ContractID

		/* added for Nick's 10k vendors (run time 2:20 for the 10k */
		drop table T_Takeda_KWS_2_ARB_TCOMPANY_ContractID

			select * into T_Takeda_KWS_2_ARB_TCOMPANY_ContractID
			from V_Takeda_KWS_2_ARB_TCOMPANY_ContractID

		drop table T_Takeda_KWS_2_ARB_Tag_ContractID

			select * into T_Takeda_KWS_2_ARB_Tag_ContractID
			from V_Takeda_KWS_2_ARB_Tag_ContractID

		drop table T_Takeda_KWS_2_ARB_TCOMPANYCountry_ContractID

			select * into T_Takeda_KWS_2_ARB_TCOMPANYCountry_ContractID
			from V_Takeda_KWS_2_ARB_TCOMPANYCountry_ContractID	

		drop table T_Takeda_KWS_2_ARB_InternalPartner_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_ARB_InternalPartner_ContractID
			from [dbo].[V_Takeda_KWS_2_ARB_InternalPartner_ContractID] 
	/* 2 - LINC */
		drop table T_Takeda_KWS_2_LNC_TCOMPANY_ContractID

			select * into T_Takeda_KWS_2_LNC_TCOMPANY_ContractID
			from V_Takeda_KWS_2_LNC_TCOMPANY_ContractID

		drop table T_Takeda_KWS_2_LNC_InternalPartner_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_LNC_InternalPartner_ContractID
			from [dbo].[V_Takeda_KWS_2_LNC_InternalPartner_ContractID] 
			/*	drop table T_Takeda_KWS_2_LNC_TPRODUCT_ContractID

			select * into T_Takeda_KWS_2_LNC_TPRODUCT_ContractID
			from dbo.[V_Takeda_KWS_2_LNC_TPRODUCT_ContractID] */

	/* 2 - JPS */

		drop table T_Takeda_KWS_2_JPS_TPRODUCT_ContractID

			select * into T_Takeda_KWS_2_JPS_TPRODUCT_ContractID
			from V_Takeda_KWS_2_JPS_TPRODUCT_ContractID

		drop table T_Takeda_KWS_2_JPS_TCOMPANY_ContractID

			select * into T_Takeda_KWS_2_JPS_TCOMPANY_ContractID
			from V_Takeda_KWS_2_JPS_TCOMPANY_ContractID	

		drop table T_Takeda_KWS_2_JPS_TCOMPANYCountry_ContractID

			select * into T_Takeda_KWS_2_JPS_TCOMPANYCountry_ContractID
			from V_Takeda_KWS_2_JPS_TCOMPANYCountry_ContractID	

		drop table T_Takeda_KWS_2_JPS_InternalPartner_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_JPS_InternalPartner_ContractID
			from [dbo].[V_Takeda_KWS_2_JPS_InternalPartner_ContractID] 

	/*		CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_2_CNT_InternalPartner_ContractID
			ON T_Takeda_KWS_2_CNT_InternalPartner_ContractID (CONTRACTID) */
		drop table T_Takeda_KWS_2_JPS_Territories_ContractID /* T_Takeda_KeyWordSearch_Results_TCOMPANY_ContractID */

			select * into T_Takeda_KWS_2_JPS_Territories_ContractID
			from [dbo].[V_Takeda_KWS_2_JPS_Territories_ContractID] 
...
/**********************************/	
	...
	/* 3 - Contiki */
		drop table T_Takeda_KWS_3_CNT_TProduct_ContractID_Extended

			select * into T_Takeda_KWS_3_CNT_TProduct_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_CNT_TProduct_ContractID_Extended]

			CREATE CLUSTERED INDEX T_Takeda_KWS_3_CNT_TProduct_ContractID_Extended
			ON T_Takeda_KWS_3_CNT_TProduct_ContractID_Extended (CONTRACTID)

			/* 1 second run time even with 10K records ?*/
			drop table T_Takeda_KWS_3_CNT_TCompany_ContractID_Extended /* match level e.g. '1 - EXACT'  */

				select * into T_Takeda_KWS_3_CNT_TCompany_ContractID_Extended
				from dbo.[V_Takeda_KWS_3_CNT_TCompany_ContractID_Extended]

	/* 3 - Ariba */

		drop table T_Takeda_KWS_3_ARB_TCompany_ContractID_Extended /* match level e.g. '1 - EXACT'  */

			select * into T_Takeda_KWS_3_ARB_TCompany_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_ARB_TCompany_ContractID_Extended]

		drop table T_Takeda_KWS_3_ARB_TProduct_ContractID_Extended

			select * into T_Takeda_KWS_3_ARB_TProduct_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_ARB_TProduct_ContractID_Extended]

			CREATE CLUSTERED INDEX T_Takeda_KWS_3_ARB_TProduct_ContractID_Extended
			ON T_Takeda_KWS_3_ARB_TProduct_ContractID_Extended (CONTRACTID)

	/* 3 - JPS */
		drop table T_Takeda_KWS_3_JPS_TProduct_ContractID_Extended

			select * into T_Takeda_KWS_3_JPS_TProduct_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_JPS_TProduct_ContractID_Extended]

		drop table T_Takeda_KWS_3_JPS_TCompany_ContractID_Extended

			select * into T_Takeda_KWS_3_JPS_TCompany_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_JPS_TCompany_ContractID_Extended]			
	/* 3 - LNC */

		drop table T_Takeda_KWS_3_LNC_TCompany_ContractID_Extended /* match level e.g. '1 - EXACT'  */

			select * into T_Takeda_KWS_3_LNC_TCompany_ContractID_Extended
			from dbo.[V_Takeda_KWS_3_LNC_TCompany_ContractID_Extended]

/**********************************/
/* LEVEL 4 - Prod / DESCRIPTION */
/**********************************/

	drop table T_Takeda_KWS_4_CNT_TProduct_ContractID

		select * into T_Takeda_KWS_4_CNT_TProduct_ContractID
		from V_Takeda_KWS_4_CNT_TProduct_ContractID
	
	drop table T_Takeda_KWS_4_ARB_TProduct_ContractID

		select * into T_Takeda_KWS_4_ARB_TProduct_ContractID
		from V_Takeda_KWS_4_ARB_TProduct_ContractID
...
/* description , run time 2:45 min*/
	drop table T_Takeda_KWS_5c_CNT_DESCRIPTION_ContractID 

		select * into T_Takeda_KWS_5c_CNT_DESCRIPTION_ContractID
		from V_Takeda_KWS_5c_CNT_DESCRIPTION_ContractID

	drop table T_Takeda_KWS_5c_ARB_DESCRIPTION_ContractID
		/* currently excludes company for Nick's 10,000 entries */
		select * into T_Takeda_KWS_5c_ARB_DESCRIPTION_ContractID
		from V_Takeda_KWS_5c_ARB_DESCRIPTION_ContractID
		
		/* run time 39 seconds for Nick's 10K vendors */
	drop table T_Takeda_KWS_5c_JPS_DESCRIPTION_ContractID

		select * into T_Takeda_KWS_5c_JPS_DESCRIPTION_ContractID
		from V_Takeda_KWS_5c_JPS_DESCRIPTION_ContractID

	drop table T_Takeda_KWS_5c_LNC_DESCRIPTION_ContractID

		select * into T_Takeda_KWS_5c_LNC_DESCRIPTION_ContractID
		from V_Takeda_KWS_5c_LNC_DESCRIPTION_ContractID
	/*	CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_4_JPS_DESCRIPTION_ContractID
		ON  T_Takeda_KWS_4_JPS_DESCRIPTION_ContractID (CONTRACTID) */

/**********************************/
	/* 6 - UNION */
/**********************************/
	
		drop table T_Takeda_KWS_6_CNT_ContractID_UNION

			select * into T_Takeda_KWS_6_CNT_ContractID_UNION
			from V_Takeda_KWS_6_CNT_ContractID_UNION

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_6_CNT_ContractID
			ON  T_Takeda_KWS_6_CNT_ContractID_UNION (CONTRACTID)

		/* Ariba */
		/* added for Nick's 10k vendors */
		drop table T_Takeda_KWS_6_ARB_ContractID_UNION

			select * into T_Takeda_KWS_6_ARB_ContractID_UNION
			from V_Takeda_KWS_6_ARB_ContractID_UNION

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_6_ARB_ContractID
			ON  T_Takeda_KWS_6_ARB_ContractID_UNION (CONTRACTID)

		drop table T_Takeda_KWS_6_LNC_ContractID_UNION

			select * into T_Takeda_KWS_6_LNC_ContractID_UNION
			from V_Takeda_KWS_6_LNC_ContractID_UNION

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_6_LNC_ContractID_UNION
			ON  T_Takeda_KWS_6_LNC_ContractID_UNION (CONTRACTID)


		drop table T_Takeda_KWS_6_JPS_ContractID_UNION

			select * into T_Takeda_KWS_6_JPS_ContractID_UNION
			from V_Takeda_KWS_6_JPS_ContractID_UNION

			CREATE UNIQUE CLUSTERED INDEX T_Takeda_KWS_6_JPS_ContractID_UNION
			ON  T_Takeda_KWS_6_JPS_ContractID_UNION (CONTRACTID)
...
/**********************************/
	...
	/* CNT */		/* run time 1 second at 10K vendor rows */
		drop table T_Takeda_KWS_7_CNT_ContractID_SummaryByContractID

			select * into T_Takeda_KWS_7_CNT_ContractID_SummaryByContractID
			from V_Takeda_KWS_7_CNT_ContractID_SummaryByContractID

	/* ARB */		/* run time 2 sec at 10K rows */
		drop table T_Takeda_KWS_7_ARB_ContractID_SummaryByContractID

			select * into T_Takeda_KWS_7_ARB_ContractID_SummaryByContractID
			from V_Takeda_KWS_7_ARB_ContractID_SummaryByContractID

	/* LNC */
		drop table T_Takeda_KWS_7_LNC_ContractID_SummaryByContractID

			select * into T_Takeda_KWS_7_LNC_ContractID_SummaryByContractID
			from V_Takeda_KWS_7_LNC_ContractID_SummaryByContractID
	
	/* JPS */		/* run time 9 seconds at 10K rows */
		drop table T_Takeda_KWS_7_JPS_ContractID_SummaryByContractID

			select * into T_Takeda_KWS_7_JPS_ContractID_SummaryByContractID
			from V_Takeda_KWS_7_JPS_ContractID_SummaryByContractID	

/**********************************/
/* 9 - FINAL / COMBINED */
/**********************************/
/* EXEC [dbo].[Takeda_1DAILY_03DataLoad_KWS] T_Takeda_KWS_0_ContikiView_CNT */
	drop table T_Takeda_KWSR_1_CNT_ARB /* and JPS */
	
		select * into T_Takeda_KWSR_1_CNT_ARB
		from [V_Takeda_KWSR_1_CNT_ARB] 
		/* WHERE 
			[Status] = 'Active' */
		/*	and ([Company Names] <> 'Intercompany Takeda (Two or more Takeda Entities)'
...
/*
select * from V_Takeda_KWSR_0_LNC where CONTRACTID is null


exec Takeda_KeyWordSearch


select *
from V_Takeda_KWSR_1_CNT_ARB where CONTRACTID is null
group by contractid
having COUNT(*)>1
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_FullText_AdhocNewProducts]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KWS_FullText_AdhocNewProducts] 
...
END 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_PrdGrpUpload_CNT_Desc_TableAddNewItems]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_KWS_PrdGrpUpload_CNT_Desc_TableAddNewItems]
...
/* WHEN NEW PRODUCTS ARE ADDED, or old ones modified, run the procedure WITHOUT the date filter  to make sure the product is tagged in older records too */
	...
	SELECT /* top 100 */ PRODUCTGROUPID, PRODUCTGROUP 
	FROM V_Takeda_VPRODUCTGROUP 
	WHERE productgroup in (select PRODUCTGROUPNAME from T_Takeda_TPRODUCTGROUP_AddNewItems)
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_PrdGrpUpload_CNT_FullText_TableAddNewItems]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_KWS_PrdGrpUpload_CNT_FullText_TableAddNewItems]
AS
/* Notes:
Currently excluding lots of names with special chars to be sure
retest
...
BEGIN
	...
	from V_Takeda_VPRODUCTGROUP 
	WHERE 
	productgroup in (select PRODUCTGROUPNAME from T_Takeda_TPRODUCTGROUP_AddNewItems)
...
END 
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_PrdGrpUpload_Execute_TableAddNewItemsAndDescFullTxt]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KWS_PrdGrpUpload_Execute_TableAddNewItemsAndDescFullTxt]

AS

BEGIN

	EXEC [dbo].[Takeda_KWS_PrdGrpUpload_TableAddNewItems]
	EXEC [dbo].[Takeda_KWS_PrdGrpUpload_CNT_FullText_TableAddNewItems]
	EXEC [dbo].[Takeda_KWS_PrdGrpUpload_CNT_Desc_TableAddNewItems]
	EXEC Takeda_KWS_UpdateProductAI
	/* Ariba, JPS? */

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_PrdGrpUpload_TableAddNewItems]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KWS_PrdGrpUpload_TableAddNewItems]
...
BEGIN
	...
	[dbo].[T_Takeda_TPRODUCTGROUP_AddNewItems] n
	...
	truncate table T_Takeda_TPRODUCTGROUP_AddNewItems /* addition complete */

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_PrdGrpUpload_zDesc_Adhoc_SingleName]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_KWS_PrdGrpUpload_zDesc_Adhoc_SingleName] ( @Keyword as VARCHAR(255))
AS
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_KWS_UpdateProductAI]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_KWS_UpdateProductAI]
...
update T_Takeda_TPRODUCT_ACTIVEINGREDIENT
...
INSERT INTO [dbo].[TPRODUCTGROUP]
	...
	select distinct 
		...
		, null /* PARENTID */
			...
			from T_Takeda_TPRODUCT_ACTIVEINGREDIENT
...
INSERT INTO [dbo].[TPRODUCTGROUP]
	...
	from T_Takeda_TPRODUCT_ACTIVEINGREDIENT pai 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_AgreementType_MSAtoSA]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE  PROCEDURE [dbo].[Takeda_Maintenance_AgreementType_MSAtoSA](
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_AgreementTypes_CorrectAgreementTypeByKeywords]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Maintenance_AgreementTypes_CorrectAgreementTypeByKeywords]

AS

BEGIN

/* #369 - Three new Takeda Pharma Vertrieb Categories */
insert into T_Takeda_AgreementType
...
, null /* linc main type */
from TAGREEMENT_TYPE
where [agreement_typeid] not in (select Agr_Typeid from T_Takeda_AgreementType)

delete from T_Takeda_AgreementType 
where Agr_Typeid not in (select agreement_typeid from TAGREEMENT_TYPE)

update ta 
set ta.AgrName = a.AGREEMENT_TYPE
from T_Takeda_AgreementType  ta inner join tagreement_type a on ta.agr_typeid = a.agreement_typeid
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_ContractStatus_CheckAndCorrection]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Maintenance_ContractStatus_CheckAndCorrection]

AS 

BEGIN

/* Please note weekly job [dbo].[Takeda_3STNIGHT_CorrectContractStatus] */
...
/* AWARDED */
/* [dbo].[Takeda_1HOURLY_SetToAwarded_DeleteAwardedDate] */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_DeleteOldUnattachedUsers]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_DeleteOldUnattachedUsers_UserID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_Maintenance_DeleteOldUnattachedUsers_UserID] (
@USERID bigint
, @EMPLOYEEID bigint
, @PERSONID bigint)
...
select 'exec [dbo].[Takeda_Maintenance_DeleteOldUnattachedUsers_UserID] ' 
...
and USERID in (select userid from V_Takeda_UserID_CountractRoleCount where Count_ACL is null)
and USERINITIAL not like '%admin%'
and USERINITIAL not like '%contiki%'
...
DECLARE @RESULTSTRING AS VARCHAR(255)
	...
	IF NOT EXISTS ( select * from v_takeda_vuser 
			where 
			userid = @USERID
...
(0 rows affected)
Msg 547, Level 16, State 0, Procedure Takeda_Maintenance_DeleteOldUnattachedUsers_UserID, Line 101 [Batch Start Line 5]
The DELETE statement conflicted with the REFERENCE constraint "TUSER_FK_EMPLOYEEID". The conflict occurred in database "Contiki_app", table "dbo.TUSER", column 'EMPLOYEEID'.
The statement has been terminated.

*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_DptCodesIP4DigitMergeUpdate]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_Maintenance_DptCodesIP4DigitMergeUpdate]

as
print 'x'
/*
exec Takeda_MergeDepartments 10087
...
from [dbo].[T_Takeda_Blueprint_CompanyReport] c 
	inner join [dbo].[T_Takeda_Blueprint_Mapping] m 
		on m.[Bp_CompanyQuickRef] = C.[QuickRef]
	
	inner join [dbo].[V_Takeda_VDepartment_ParsedDpt_InternalPartner_ACTIVE] d 
		on dbo.Takeda_RemoveNonAlphaCharacters(d.[InternalPartner_Clean])  like  dbo.Takeda_RemoveNonAlphaCharacters(c.[Company Name]) + '%'
		OR dbo.Takeda_RemoveNonAlphaCharacters(c.[Company Name])  like  dbo.Takeda_RemoveNonAlphaCharacters(d.[InternalPartner_Clean]) + '%'
	...
	update [dbo].[T_Takeda_Blueprint_Mapping]
	set [Cnt_CompanyID] = null, [Cnt_CompanyName] = null
	where 
	[Cnt_CompanyID] in (select [Cnt_CompanyID] from [T_Takeda_Blueprint_Mapping]
	group by [Cnt_CompanyID]
	having COUNT([Cnt_CompanyID])>1)

	select *
	from [dbo].[T_Takeda_Blueprint_Mapping] 
	...
	create view V_Takeda_Blueprint_IP

	as

		select d.*, C.*
	from   [dbo].[V_Takeda_VDepartment_ParsedDpt_InternalPartner] d 	
		left join [dbo].[T_Takeda_Blueprint_Mapping] m
			on  m.[Cnt_CompanyID] = d.[DEPARTMENTID]
		left join  [dbo].[T_Takeda_Blueprint_CompanyReport] c 
	...
	WHERE d.internalpartnerstatusflag = -1
		...
		OR c.[Company Name] like  '%' + d.[InternalPartner_Clean] + '%'
			left join [dbo].[T_Takeda_Blueprint_CompanyReport] c on 
	inner join
	WHERE /* c.[Company Name] like '%baxalta belg%' */
	and c.[Company status (code)]='ACTIVE'
...
from 
 [dbo].[T_Takeda_Blueprint_Mapping] m inner join [dbo].[T_Takeda_Blueprint_CompanyReport] c 
 on m.[Bp_CompanyQuickRef] = C.[QuickRef]

insert 
into [dbo].[T_Takeda_Blueprint_Mapping] 
select [QuickRef] as [QuickRef]
, [Company Name] as [Company Name], null, null
from [dbo].[T_Takeda_Blueprint_CompanyReport]
*/
/*
select * from [V_Takeda_VDepartment_ParsedDpt_InternalPartner] where DEPARTMENT like '%baxalta manuf%'

select * from [T_Takeda_Blueprint_Mapping] where [Bp_CompanyName] like '%baxalta manuf%'
...
*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_Misc]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE procedure [dbo].[Takeda_Maintenance_Misc]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_restoreTpostDiscussion]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Maintenance_restoreTpostDiscussion]
...
insert into TDISCUSSION (
 /*[DISCUSSIONID] */
  ...
  and MIK_VALID = 1
  and OBJECTID in (select contractid from V_Takeda_vcontract 
...
SELECT TOP (1000) [DISCUSSIONID]
      ...
      ,[MIK_VALID]
      ,[DISCUSSIONTYPEID]
  FROM [Contiki_app].[dbo].[TDISCUSSION]
  ...
  select * from v_takeda_vDOCUMENT where DOCUMENTID = 202004
  ...
  */
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_SAP_ID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create  PROCEDURE [dbo].[Takeda_Maintenance_SAP_ID]
...
from [dbo].[T_Takeda_VCompany] c 
	inner join V_Takeda_Ariba_Suppliers_SAPID_Country_VALID s 
...
where 
		...
		and c.companyid_LN in (select companyid from TTENDERER 
					where CONTRACTID in (select CONTRACTID from T_Takeda_ALL where InternalPartners_DptCodeList like '%,fr%'))
...
from tcompany c
	inner join [dbo].[T_Takeda_VCompany] cv 
...
END
/*
select * from [T_Takeda_Ariba_Suppliers_SAPID_Country] where SupName_SAP like 'Amgen%'
	inner join V_Takeda_Ariba_Suppliers_SAPID_Country_VALID s 
		on cv.[Company_LettersNumbersOnly_UPPER] like s.[Sup_LettersNumbersOnly_UPPER]  + '%' 
		OR s.[Sup_LettersNumbersOnly_UPPER] like   cv.[Company_LettersNumbersOnly_UPPER] + '%' 
		*/
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_TerritoryHashTags]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
[dbo].[Takeda_Maintenance_TerritoryHashTags]

as 
/*
update d set d.department_code = replace(d.DEPARTMENT_CODE,d.DEPARTMENT_CODE, d.DEPARTMENT_CODE + '#GEM')
from tdepartment d inner join T_Takeda_Hierarchy h on d.DEPARTMENTID = h.DEPARTMENTID
where 
L1 = 'GEM' 
...
*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Maintenance_Usergroup_Department_Cleanup]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_Maintenance_Usergroup_Department_Cleanup]
...
SELECT 
  ...
  FROM V_Takeda_VDEPARTMENT_VUSERGROUP
  WHERE 
...
/*	and DEPARTMENT like '%delete%' */
	...
	from V_Takeda_VDEPARTMENT_VUSERGROUP u
		inner join V_Takeda_VDEPARTMENT_VUSERGROUP d on u.PARENTID = d.departmentid
...
from T_Takeda_VCompany c 
	inner join T_Takeda_Ariba_Suppliers_SAPID_Country_AllFields a
...
select *
 from T_takeda_vcompany c inner join [V_Takeda_VCOMPANY_DUPLICATES] d
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_MergeDepartments]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_MergeDepartments](
...
/* Check if valid input parameters passed */
/*
select * from TDEPARTMENT where DEPARTMENT like '%takeda pharma ag%'
select * from tusergroup where usergroup like '%takeda pharma ag%'
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_MIG_DOWNLOAD]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_MIG_DOWNLOAD]
...
select migfolder  + MigFolder_Sub
	, SUM([FileSize]) as Size
			, COUNT(*) 
from V_Takeda_LNC_GoldStandard_Documents a
...
select migfolder, Contract_type, agreement_type, COUNT(*)
 from V_Takeda_LNC_GoldStandard_documents
...
select COUNT(*) from V_Takeda_LNC_GoldStandard_Documents

select * from V_Takeda_LNC_GoldStandard_Documents where MigFolder = 'other'
...
group by migfolder
			...
			, a.[contractstatus] 
				...
				if OBJECT_ID('T_Takeda_LNC_GoldStandard_Documents') is not null 
			drop table T_Takeda_LNC_GoldStandard_Documents
			 select * into T_Takeda_LNC_GoldStandard_Documents from V_Takeda_LNC_GoldStandard_Documents  
...
/* '2YRS' O:\Mig_FileShare\INACTIVE_Last2Yrs*/
	...
	WHERE 
	fileid in (
		select FileID from V_Takeda_VDOCUMENT 
		where OBJECTTYPEID = 1 /* contract */ 
			and MIK_VALID = 1
			and OBJECTID in (
			select CONTRACTID
...
select distinct filetype from TFILE where FileType not like '%1%'
	...
	filetype like '%1%'
			...
			WHERE filetype like '.%1'
/*
select COUNT(*) from V_Takeda_VCONTRACT
select * from V_Takeda_VCONTRACT where CONTRACTID not in (select CONTRACTID from T_Takeda_ALL)
select COUNT(*) from T_Takeda_ALL /*67238*/
select COUNT(*) from T_Takeda_ALL_xt

select * from TCONTRACT where STRATEGYTYPEID is null
*/
end

select * from T_Takeda_ALL_xt where Number = 'Case-00000785'
select * from TCONTRACT where contractNumber = 'Case-00000785'

update T_Takeda_ALL set Title_InclTopSecret = 'Shareholder consent' where CONTRACTID = 103928

*/

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupDeactivateUnused]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/****** Script for SelectTopNRows command from SSMS  ******/
create procedure [dbo].[Takeda_ProductGroupDeactivateUnused]
...
BEGIN
	...
	set mik_valid = 0 
	  where productgroupid in (SELECT productgroupid
			FROM [Contiki_app].[dbo].[V_Takeda_VProductGroupIsUsed]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_ArchiveLogTable]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create procedure [dbo].[Takeda_ProductGroupUpload_ArchiveLogTable]
...
BEGIN
	...
	set mik_valid = 0 
	  ...
	  insert into T_Takeda_Product_Upload_History
	  select * from T_Takeda_Product_Upload
	  where [Uploaded_DateTime] <  dateadd(dd,-14,GETDATE()) 

	  delete from T_Takeda_Product_Upload
	  where [Uploaded_DateTime] <  dateadd(dd,-14,GETDATE()) 
	 
	/* select * into T_Takeda_Product_Upload_History from T_Takeda_Product_Upload 
	where [Uploaded_DateTime] <  dateadd(dd,-14,GETDATE()) */

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_Description_ADHOC]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_ProductGroupUpload_Description_ADHOC]
AS
			...
			IF EXISTS (
				...
				PRINT ' exists at least 1 record'
					...
					WHILE @@FETCH_STATUS = 0 BEGIN
						...
						PRINT @CONTRACTNUMBER
						PRINT @DESCRIPTION
							EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID @OBJECTID, @PRODUCTGROUPID, 1 /* OBJECTTYPEID */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_Description_AdhocWrongSql]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[Takeda_ProductGroupUpload_Description_AdhocWrongSql]
AS
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_ObjectidProductgroupID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/* @OBJECTTYPEID = 1 */
	...
	/* @OBJECTTYPEID = 1 AND 4 - add to main contract if amendment has a hit */
		...
		*/
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_ObjectidProductgroupID_ARIBA]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_ProductGroupUpload_ObjectidProductgroupID_ARIBA](
...
BEGIN
			...
			IF EXISTS (SELECT 1 FROM T_Takeda_Ariba_Products_In_Contracts where 
			...
			INSERT INTO T_Takeda_Ariba_Products_In_Contracts ([CONTRACTNUMBER],[CONTRACTINTERNALID],[PRODUCTGROUPID],[Source],[MatchLevel],[DateAdded]) 
								values (@CONTRACTNUMBER,@CONTRACTINTERNALID, @PRODUCTGROUPID, @SOURCENAME, @MATCHLEVEL,GetDate())
				
				BEGIN
					SET @RESULTSTRING = 'Contract Record inserted successfully!'+
					(select CONTRACTNUMBER + ': '+[Contract Description] from T_Takeda_AribaDump
					where CONTRACTNUMBER = @CONTRACTNUMBER)
					GOTO lblEnd
				END
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_ProductGroupUpload_ObjectidProductgroupID_JPSunrise]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_ProductGroupUpload_ObjectidProductgroupID_JPSunrise](
                @CONTRACTNUMBER VARCHAR(50)
				, @CONTRACTID int
                ,@PRODUCTGROUPID bigint
				, @SOURCENAME VARCHAR(255)
...
IF NOT EXISTS ( SELECT  1
                FROM    [dbo].[T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements]
                WHERE   CONTRACTID = @CONTRACTID)
	BEGIN
		SET @RESULTSTRING = 'CONTRACTNUMBER does not exist: ' + 
			(CASE WHEN @CONTRACTNUMBER IS NULL THEN 'NULL' ELSE @CONTRACTNUMBER END)
...
BEGIN
			...
			IF EXISTS (SELECT 1 FROM [dbo].[T_Takeda_ContractData_JPSunrise_Products_In_Contracts] where 
						CONTRACTID = @CONTRACTID
						AND productgroupid = @PRODUCTGROUPID)
					BEGIN
						SET @RESULTSTRING = 'Contract Record CONTRACTNUMBER:' + @CONTRACTNUMBER
			...
			INSERT INTO  [dbo].[T_Takeda_ContractData_JPSunrise_Products_In_Contracts]([CONTRACTID],[PRODUCTGROUPID],[Source],[MatchLevel],[DateAdded]) 
								values (@CONTRACTID, @PRODUCTGROUPID, @SOURCENAME, @MATCHLEVEL,GetDate())
				
				BEGIN
					SET @RESULTSTRING = 'Contract Record inserted successfully!'+
					(select CONTRACTNUMBER + ': '+[Name of Agreement] from [dbo].[T_Takeda_ContractData_JP_Sunrise_ExecutedAgreements]
					where CONTRACTID = @CONTRACTID)
					GOTO lblEnd
				END
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Reminders_Deactivate_ContractID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_Reminders_Deactivate_ContractID](
                  @CONTRACTID bigint
                /* , @XtPrefix as VARCHAR(255) /* Wave # 01 etc... !ARIBA_WXX */ */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Scheduled_Daily]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Scheduled_Daily]
...
BEGIN
/* status problems: manually run [dbo].[Takeda_CORRECTION_STATUS] */
	...
	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'SKIPPED' + @JOBSTARTDATETIME)

/*	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'1_Start ' + @JOBSTARTDATETIME)
		
	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'1b_HourlyJob ' + @JOBSTARTDATETIME) 
		
		EXEC Takeda_Scheduled_Hourly /* while hourly job is deactivated on job runner */

	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'2_Takeda_0_DataLoad ' + @JOBSTARTDATETIME) 

		EXEC Takeda_1DAILY_0DataLoad /* target completion time around 9 minutes */

	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'3_Takeda_0_EditDataLoad ' + @JOBSTARTDATETIME)

		EXEC Takeda_1DAILY_0Edit_DataLoad

	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'4_Permissions ' + @JOBSTARTDATETIME) 
		
		EXEC [dbo].[Takeda_1DAILY_03DataLoad_KWS]

/* Auto Permissions */
 		EXEC Takeda_1DAILY_AUTODELETE /* !AUTODELETE flag action */
		EXEC Takeda_1DAILY_AUTODELETE_Deactivate_Reminders		
		EXEC Takeda_1DAILY_AUTODELETE_RemovePermissions

		EXEC Takeda_1DAILY_ConfidentialityFlag /* Confidential, Strictly Confidential, Top Secret Flags */

		EXEC Takeda_1DAILY_ACL_AddMissingPermissions_AUTO
		EXEC Takeda_1DAILY_ACL_AddRemoveAutoPermissions

		/* Public read permissions */
		EXEC Takeda_1DAILY_PUBLIC_READ_Permissions

/* set strategytype to HCP if null for appropriate contract types */	
	
		EXEC Takeda_2DAILY_UpdateStrategyTypeTakPhVertrieb

/* Sort Department fields */

		EXEC [Takeda_2DAILY_AlphaSort_TDEPARTMENT]

	INSERT INTO T_Takeda_JobLog values('Scheduled_Daily', getdate(),'9_End '+ @JOBSTARTDATETIME) 

END

/* User domain in title fields - deactivated because all is Takeda now */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Scheduled_Hourly]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[Takeda_Scheduled_Hourly]
...
BEGIN
	...
	INSERT INTO T_Takeda_JobLog values('Sched_Hrly', getdate(),'0_Start_9End')
/*	/* due to a mess-up the hourly job ran the daily job hourly and we had to disable the hourly job
	...
	but in the meantime, this hourly job is run once a day via the scheduled_daily job */
		...
		EXEC Takeda_1HOURLY_CancelCheckoutAllPdfs /* run time 0 sec */
		...
		EXEC Takeda_1HOURLY_SetToAwarded_DeleteAwardedDate /* run time 0 sec */
		/* awarded date presence leads to funny errors when trying to add a date that is prior to the awarded date */
	
		/* Takeda_489_NoReviewDateReminder no idea why this is commented out */

*/
	 INSERT INTO T_Takeda_JobLog values('Sched_Hrly', getdate(),'9_End')

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Scheduled_Monthly]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Scheduled_Monthly]
...
BEGIN
	...
	INSERT INTO T_Takeda_JobLog values('Scheduled_Monthly', getdate(),'SKIPPED') 
/*	INSERT INTO T_Takeda_JobLog values('Scheduled_Monthly', getdate(),'0_Start') 
	EXEC [Takeda_4MONTHLY_AlphaSort_TUSERGROUP] 
	/* EXEC [Takeda_4MONTHLY_AlphaSort_TDEPARTMENT] now daily */
	EXEC [Takeda_4MONTHLY_AlphaSort_AGREEMENT_TYPE]
	EXEC [Takeda_4MONTHLY_DeleteDocsRecycleBin] /* CMA Contiki script */
	EXEC MaintainSplitAuditTrail /* Coupa function to move all records older than 1 month to audit trail history */
		...
		DELETE FROM T_Takeda_JobLog 
			WHERE (RunTime < GETDATE() - 30)

	INSERT INTO T_Takeda_JobLog values('Scheduled_Monthly', getdate(),'9_End') 
*/
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Scheduled_SaturdayNight]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Scheduled_SaturdayNight]
...
BEGIN
	...
	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'SKIPPED') 
/*	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'00_START') 
		EXEC Takeda_3SATNIGHT_FlagNoFiles
		EXEC Takeda_3SATNIGHT_RemapTT_AlphaToRegion
		EXEC Takeda_3SATNIGHT_AddCurrentIP_ForActiveContracts
		EXEC Takeda_3SATNIGHT_FlagBlank_CountryCityStreet
		EXEC Takeda_3SATNIGHT_Reminders_Deactivate_ExpIntercompany /* deactivate reminders if contract has expiry date or is intercompany, or CDA */
		EXEC Takeda_3SATNIGHT_AddDocumentTag_FullText

	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'01_ContractStatus') 

		EXEC Takeda_3SATNIGHT_CorrectContractStatus

	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'06_FTxt_S') 
		
		EXEC [Takeda_3SATNIGHT_PrdGrpUpload_CNT_FullText] /* dynavit */ /* takes about 5 min after revision and improvement */

	/* Description prod group upload moved from weekly to sat night due to long run time from 1 hr to 6 hrs */
	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'7_Prod_Desc') 

		EXEC Takeda_3SATNIGHT_PrdGrpUpload_CNT_Description /* Last run time on 28-Oct-2018 was 1:10 */			
				
	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'08_FTxt_CNKT_E')

		EXEC [Takeda_3SATNIGHT_PrdGrpUpload_ARB_Description]
		EXEC [Takeda_3SATNIGHT_PrdGrpUpload_JPS_Description]

	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'08_FTxt_ARIBA_E')

	INSERT INTO T_Takeda_JobLog values('Scheduled_SatNight', getdate(),'09_END') 
	*/
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_Scheduled_Weekly]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_Scheduled_Weekly]

AS

BEGIN

	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'SKIPPED') 
/*	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'1Start') 
		EXEC Takeda_2WEEKLY_Maintenance_AgreementTypes
		EXEC Takeda_2WEEKLY_RemapTT_IPToTT
		EXEC Takeda_2WEEKLY_HardcopyArchiving_DeleteTTEntries
		EXEC Takeda_2WEEKLY_ReplaceUnderscoresWithDashes
		EXEC Takeda_2WEEKLY_CompanyCleanup_TakedaIntercompany

		EXEC [Takeda_2WEEKLY_RenameDTitleToCTitle]

/*	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'6_Prod_Desc') 

		EXEC Takeda_2WEEKLY (now3SATNIGHT)_ProductGroupUpload_Description 		/* Last run time on 28-Oct-2018 was 1:10 */		MOVED due to 6:00:44 run time length on 03-Apr-2020
*/
	INSERT INTO T_Takeda_JobLog values('Scheduled_Weekly', getdate(),'9_END') 
	*/
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_TablesColumnsAddNewItems]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_TablesColumnsAddNewItems]
...
select * from INFORMATION_SCHEMA.COLUMNS where [COLUMN_NAME] like '%eadin%'
	...
	/* add new table */
	 insert into [dbo].[T_Takeda_ContikiTables] ([TblVwObjectID]
	, [TblVwName]
	, [TblVwComments]
	, [TblVwRelationship]
	, [TblVwBaseTableName]) 
	...
	from V_Takeda_ContikiTablesColumns
	where tableviewname in (
		'TFILE'
		 ) 

	/* Add New Columns */
		insert into T_Takeda_ContikiTablesColumns
		( 
      [TableName]
      ,[ColumnName]
      ,[MigrateYN]
      ...
      /*,[ColFieldID] */
			...
			/*, m.ColFieldID */
			from 
			[dbo].[V_Takeda_ContikiTablesColumns] m /* all tables */
			where m.objectid in (select [TblVwObjectID] from V_Takeda_ContikiTables)
			AND m.ObjectID not in (select TVobjectid from T_Takeda_ContikiTablesColumns )	
	...
	from [dbo].[T_Takeda_ContikiTablesColumns] c inner join 
	[dbo].[V_Takeda_ContikiTablesColumns] tc on C.[TableName] = tc.TableViewName 
	...
	/* add sample values */
		...
		inner join V_Takeda_ContikiTablesColumns m on isc.TABLE_NAME = m.TableViewName
		inner join [dbo].[T_Takeda_ContikiTables] t on t.[TblVwName] = m.TableViewName
		left join [dbo].[T_Takeda_ContikiTablesColumns] c on isc.TABLE_NAME = c.tablename 
	...
	/* add into sample value table */
		INSERT INTO 
		T_Takeda_ContikiTablesColumns_SampleValue
	...
	/* update column table with sample values for new colunmns */
		...
		from [dbo].[T_Takeda_ContikiTablesColumns] t inner join [dbo].[T_Takeda_ContikiTablesColumns_SampleValue] ts
		on t.[TableName] = ts.[TableName] and t.columnname = ts.ColumnName
		where t.[ValueTop1] is null and ts.[ValueTop1] is not null	

	
		delete from [dbo].[T_Takeda_ContikiTablesColumns_SampleValue]

		update [dbo].[T_Takeda_ContikiTablesColumns] set isblank = -1 where [ValueTop1] =''
...
/* gold std value */
		...
		FROM INFORMATION_SCHEMA.COLUMNS isc
		inner join V_Takeda_ContikiTablesColumns m on isc.TABLE_NAME = m.TableViewName
		inner join [dbo].[V_Takeda_ContikiTables] t on t.[TblVwName] = m.TableViewName
		left join [dbo].[T_Takeda_ContikiTablesColumns] c on isc.TABLE_NAME = c.tablename 
		and isc.COLUMN_NAME = C.[ColumnName]
		/* WHERE m.tableviewname in (
		'TTENDERER') */
		WHERE c.ValueGoldStd is null
		 ...
		 INSERT INTO 
		T_Takeda_ContikiTablesColumns_GoldStd	
...
/* contractnumber = 'TEST-00000080' */ 
	...
	/* update column table with sample values for new colunmns */
		...
		from [dbo].[T_Takeda_ContikiTablesColumns] t inner join [dbo].[T_Takeda_ContikiTablesColumns_GoldStd] ts
		on t.[TableName] = ts.[TableName] and t.columnname = ts.ColumnName
		...
		delete from [dbo].[T_Takeda_ContikiTablesColumns_GoldStd]

		/* update [dbo].[T_Takeda_ContikiTablesColumns] set isblank = -1 where [ValueGoldStd] ='' */

	
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_TagUpload_DocumentID]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_TagUpload_DocumentID](
                @TAGID bigint
...
/* Run Wrapper: Takeda_3SATNIGHT_AddDocumentTag_FullText */
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_UTILITY_Remap_AgreementTypes]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_UTILITY_Remap_AgreementTypes]
...
/* Instructions:
0. SQL Server, run this command: truncate table T_Takeda_Remap_AgreementType
1. Access Adhoc DB: Paste data into Adhoc.accdb table tbl_AgreementTypeRemap_DataINPUT
2. Access Adhoc DB: Open qry_T_Takeda_Remap_AgreementType for append script
...
5. Access: qry_T_Takeda_Remap_AgreementType_FINALCHECK shows results
*/


update r
set r.ContractIDInput = c.contractid
from T_Takeda_Remap_AgreementType r inner join tcontract c 
...
from T_Takeda_Remap_AgreementType r inner join TAGREEMENT_TYPE a 
...
from T_Takeda_Remap_AgreementType r inner join tagreement_type a 
on dbo.Takeda_RemoveNonAlphaCharacters(r.agreementtypeinput_N) 
= dbo.Takeda_RemoveNonAlphaCharacters(a.AGREEMENT_TYPE)
where (r.Agreement_TypeID_N is null or r.Agreement_TypeID_N <> a.agreement_typeid)

/* check */
select * from dbo.T_Takeda_Remap_AgreementType
...
from tcontract c inner join T_Takeda_Remap_AgreementType r 
...
GO
/****** Object:  StoredProcedure [dbo].[Takeda_VENDORSCRIPT_MaintenanceCheck_MissingACL]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE procedure [dbo].[Takeda_VENDORSCRIPT_MaintenanceCheck_MissingACL]
...
THE_FINAL_END:
GO
/****** Object:  StoredProcedure [dbo].[Takeda_VENDORSCRIPT_RebuildIndicesStats]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE procedure [dbo].[Takeda_VENDORSCRIPT_RebuildIndicesStats]
...
WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME in('Contiki_app', 'Contiki_wf')
ORDER BY TABLE_NAME
OPEN TABLES
...
SET NOCOUNT OFF
GO
/****** Object:  StoredProcedure [dbo].[Takeda_z_174_HourlySetWfUsersToActiveWhereUserActive]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
create PROCEDURE [dbo].[Takeda_z_174_HourlySetWfUsersToActiveWhereUserActive]
...
BEGIN
  ...
  update w
  set w.mik_valid = 1
  from [Contiki_wf].[dbo].[TWorkflowUser] w 
  inner join  [Contiki_app].[dbo].[TUSER] u on u.userid = w.externaluserid
  where u.mik_valid = 1 and w.mik_valid <>1

END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_z2WEEKLY_ProductGroupUpload_Description_OLD]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
CREATE PROCEDURE [dbo].[Takeda_z2WEEKLY_ProductGroupUpload_Description_OLD]
AS
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_z2WEEKLY_ProductGroupUpload_Description_ProdNameOver6Char]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_z2WEEKLY_ProductGroupUpload_Description_ProdNameOver6Char]
AS
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_zHourly_zFlagNoFiles]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_zHourly_zFlagNoFiles] 
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_zOneTimeFix_479_AddNumberSeriesPrefix]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE procedure [dbo].[Takeda_zOneTimeFix_479_AddNumberSeriesPrefix]
...
END
GO
/****** Object:  StoredProcedure [dbo].[Takeda_zzz486_UpdatePdfFromPendingToCompleted]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[Takeda_zzz486_UpdatePdfFromPendingToCompleted]
...
GO
/****** Object:  StoredProcedure [dbo].[usp_Takeda_zSendTextEmailTEST]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[usp_Takeda_zSendTextEmailTEST]  @ServerAddr nvarchar(128),
@From nvarchar(128),
@To nvarchar(1024),
...
GO
/****** Object:  StoredProcedure [dbo].[zzzTakeda_ProductGroupUpload_FullText_Backup]    Script Date: 24 Jun 2024 08:57:54 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
create PROCEDURE [dbo].[zzzTakeda_ProductGroupUpload_FullText_Backup]
AS
/* Notes:
Currently excluding lots of names with special chars to be sure
retest
...
BEGIN
	...
	select PRODUCTGROUPID, PRODUCTGROUP from V_Takeda_AUTO_ProductGroupsToUpload where 
	blnNumHashes = 0 /* no hash, include in full text search */
	/* AND PRODUCTGROUP IN('ADCETRIS','OMNARIS') */
	AND PRODUCTGROUP LIKE '%[a-z]%' 
	AND PRODUCTGROUP NOT LIKE '% %'
		...
		PRINT 'Product Group: '  + @PRODUCTGROUP
			...
			BEGIN /* IF EXISTS */
				...
				BEGIN
					 ...
					 EXEC Takeda_ProductGroupUpload_ObjectidProductgroupID @OBJECTID, @PRODUCTGROUPID
